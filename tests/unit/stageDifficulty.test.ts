 ...
 *
 * **파루아의 저체력 은신으로 이 표를 다시 쟀다.** 체력이 절반 밑으로 떨어질 때 집중을 모두
 * 내려놓고 3초 물러서는 절이 붙었는데, 파루아는 이 검수의 바닥 로스터 셋 중 하나이자 **매 판
 * 가장 먼저 쓰러지던** 자리다. 먼저 죽지 않으니 3장 뒷부분이 눈에 띄게 나아졌고(균등 3-9가
 * 0.625 → 0.875, 3-7이 0.625 → 0.75), **몰아주기는 구멍이 통째로 사라졌다** — 자원을 받는
 * 자리가 바로 그 개체라 살아남는 절과 성장이 같은 곳에 쌓인다. 대신 1-3이 0.875 → 0.75로
 * 내려갔다: 집중을 내려놓는 값이 아직 어릴 때는 손해로만 남는다.
 */
const FLOOR_GAP: Readonly<Record<"spread" | "carry", Readonly<Record<string, number>>>> = {
  spread: {
    "1-3": 0.75, "1-4": 0.875, "1-6": 0.875, "1-7": 0.875, "1-8": 0.875, "1-9": 0.875,
    "2-1": 0.875, "2-2": 0.875, "2-3": 0.875, "2-4": 0.875, "2-5": 0.875, "2-6": 0.875,
    "2-7": 0.875, "2-8": 0.875, "2-10": 0.875,
    "3-1": 0.875, "3-2": 0.875, "3-3": 0.875, "3-4": 0.875, "3-5": 0.875, "3-6": 0.875,
    "3-7": 0.75, "3-8": 0.5, "3-9": 0.875,
  },
  // 몰아주기는 구멍이 하나도 없다 — 파루아가 자원을 받는 자리라, 살아남는 절이 붙은 뒤로는
  // 그 한 명이 끝까지 선다.
  carry: {},
};
 ...
 *
 * **파루아 조정으로 다시 쟀다.** 먼저 쓰러지던 개체가 한 번 물러서고 살아남으므로 승패는
 * 나아졌지만, **남은 체력은 오히려 내려갔다** — 예전에는 일찍 죽어 0으로 굳던 몫이 이제
 * 반쯤 깎인 채로 끝까지 서 있기 때문이다(2·3장 중반이 0.50 → 0.39). 띠 폭은 예전과 같고
 * 가운데만 옮겨 갔다 — 폭까지 넓히면 다음 조정의 체감 변화가 이 검수에 걸리지 않는다.
 */
const BASELINES = {
  "1-1": { hp: [0.50, 0.62] },
...
describe("Phaser 없는 챕터 난이도 검수", () => {
  ...
  /*
   * **키우지 않으면 막힌다 — 그 벽은 1장 중반이다.**
   *
   * 같은 로스터를 1레벨·룬 없이 세운 파티다. 한동안 이 벽이 3장까지 밀려 있었는데, 파루아가
   * 매 판 먼저 쓰러지며 2대3이 되던 구조를 저체력 은신으로 고치자 **벽이 제자리로 돌아왔다**:
   * 1-4까지는 넘고, 1-6에서 여덟 판 중 한 판만 이기며, 1-7·1-8은 한 판도 넘지 못한다.
   *
   * **정예 관문(1-5·1-10)은 이 줄 밖이다.** 셋 대신 하나가 서는 자리라 맨몸도 넘는다 — 그것이
   * 사고가 아닌 이유는 무게를 내는 축이 다르기 때문이다(그 하나의 야성).
   */
  it("성장하지 않은 파티는 1장 중반에서 막힌다", () => {
    ...
    // 앞의 넷은 맨몸으로도 배우는 구간이다.
    expect(winRateAt("1-1")).toBe(1);
    // 1-6에서 흔들리기 시작해 그 다음 둘에서 완전히 막힌다.
    expect(winRateAt("1-6")).toBeLessThanOrEqual(0.25);
    expect(winRateAt("1-7")).toBe(0);
    expect(winRateAt("1-8")).toBe(0);
    // 그 뒤로는 끝까지 열리지 않는다.
