...
export const RELICS: RelicDef[] = [
  ...
  },
  {
    ...
    passive: {
      ...
      /*
       * **반쯤 무너진 순간 한 번, 쌓아 둔 것을 다 내려놓고 물러선다.** 600 뒤에서 쏘는
       * 유리몸이라 붙은 상대에게 잡히면 그대로 무너지는데, 검수에서 이 개체는 매 판 **가장
       * 먼저** 쓰러져 바닥 파티를 2대3으로 만들었다. 은신은 표적에서 벗어나게 하므로 그 순간
       * 붙은 적이 다른 곳을 보고, 그 3초가 다시 거리를 벌 시간이 된다.
       *
       * **치르는 값은 집중 전부다.** 공짜로 빠져나오면 뒤로 물러서는 것이 기본 행동이 되어
       * 앞에 선 아군이 할 일이 없다 — 살아남은 대신 공격력도 사거리도 처음으로 돌아간다.
       * 발동 경계(최대 체력 50%)와 전투당 한 번뿐인 발동권은 스테라의 저체력 은신과 같다.
       */
      lowHpVanishSeconds: 3,
      lowHpVanishSpendsFocus: true,
      // 전용 분기가 있으므로 화면에는 이 문장이 아니라 `passiveDescription`이 지은 것이 뜬다.
      // 여기 사본은 데이터만 읽는 사람을 위한 것이라, 분기를 고칠 때 함께 고친다.
      desc: "사거리 안에서 가장 먼 적을 노리고, 적중할 때마다 집중을 얻어 공격력과 사거리가 오른다. 전투당 한 번, 체력이 절반 이하가 되면 집중을 모두 잃는 대신 3초 동안 은신한다.",
    },
    basic: {
      id: "parua-basic",
      name: "신중한 일격...! 이에요...",
