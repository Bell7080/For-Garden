...
describe("렐릭 성장 규칙", () => {
  it("현재 레벨 비용의 정확한 경계에서만 레벨업하고 원본을 변경하지 않는다", () => {
    const progress: RelicProgress = { level: 2, levelTitle: "복원체", dnaMastery: 0, heartGemSlots: [null, null, null] };
  ...
  it("최대 레벨과 재화 부족에서는 성장 상태를 만들지 않는다", () => {
    const base: RelicProgress = { level: 1, levelTitle: "복원체", dnaMastery: 0, heartGemSlots: [null, null, null] };
