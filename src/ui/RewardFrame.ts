...
export class RewardFrame extends Phaser.GameObjects.Container {
  constructor(scene: Phaser.Scene, x: number, y: number, options: { icon: CurrencyIconKey; amount: number; size?: number; state?: RewardFrameState; onClick?: () => void }) {
    super(scene, x, y);
    scene.add.existing(this);
    const size = options.size ?? 128;
    const state = options.state ?? "normal";
    const color = state === "claimable" ? COLOR.missionClaim : state === "claimed" ? 0x68717d : COLOR.accent;
