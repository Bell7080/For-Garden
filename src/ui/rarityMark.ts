 ...
 * 등급 표식.
 *
 * 등급은 별 다섯 칸이 아니라 **로마자 한 글자**다. 별은 칸을 세어야 알 수 있고 카드마다
 * 다섯 개가 반짝여 얼굴보다 먼저 눈에 들어왔다. 로마자는 한 글자라 자리를 적게 먹고,
 * 크기와 색만으로 위계가 읽힌다.
 *
 * 색과 등급의 대응은 한 곳에서만 정한다 — SSR 황금 호박, SR 보랏빛, R 청량한 하늘빛이며
 * 정보창의 등급 글자(`RARITY_GEM`)와 같은 계열이다.
 */
export const RARITY_ROMAN: Record<RelicRarity, string> = { SSR: "V", SR: "IV", R: "III" };
...
export function addRarityMark(
  ...
  const numeral = RARITY_ROMAN[rarity];
  ...
  put(0, 0, tone.halo, 0.16, 1.6);
  put(0, 0, tone.halo, 0.3, 1.24);
  put(0, 0, tone.ink, 1, 1);
}
