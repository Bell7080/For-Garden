...
/**
 * 레이드 전투의 머리 — **시즌 보스의 남은 체력 → 시계 → 점수** 순으로 위에서 쌓인다.
 *
 * 레이드 화면이 가장 크게 세우는 것이 남은 체력 줄이라, 전투 중에도 같은 줄이 맨 위에 서서
 * 지금 이 판의 점수만큼 **실시간으로 깎인다**. 시즌 화면에서 본 줄과 싸우는 동안 보는 줄이
 * 같은 모양이어야 "지금 내가 저 줄을 밀고 있다"로 읽힌다. 그래서 시계와 점수는 그 아래로
 * 한 칸씩 내려선다 — 줄을 시계 아래에 두면 가장 무거운 정보가 두 줄 밑으로 밀린다.
 *
 * 값은 Phaser 없는 표에 두어 테스트가 서로 겹치지 않는지 잰다.
 */
export const RAID_BATTLE_HUD = {
  centerX: 1080 / 2,
  /** 게이지 위의 이름표 줄(왼쪽 "남은 체력" · 오른쪽 보스 이름). */
  labelY: 30,
  bar: { y: 72, width: 860, height: 34, ticks: 7 },
  /** 게이지 아래 남은 체력 수치. 왼쪽 끝에 붙는다. */
  valueY: 110,
  /** 시계. 원래 자리(`BATTLE_CLOCK_LAYOUT.y`)에서 줄 아래로 내려선다. */
  clockY: 132,
  /** 이번 판의 점수. 시계 바로 아래다. */
  scoreY: 190,
  /** 단계 줄. 점수와 같은 높이의 왼쪽이라 가운데 수와 부딪히지 않는다. */
  phaseX: 42,
  phaseY: 206,
  depth: 90,
} as const;
