 ...
 */
/**
 * 바닥에 **머무는** 면. 여울처럼 자리에 걸린 지속 상태가 쓴다.
 *
 * `AREA_IMPACT`와 갈라 두는 이유는 그쪽이 "여기까지가 범위였다"를 한 번 벌렸다 꺼뜨리는
 * **순간 표시**이기 때문이다. 여울에 그 연출을 매초 다시 틀었더니 1초마다 물이 새로 고이는
 * 것처럼 깜빡였다 — 머무는 면은 벌어지지도 꺼지지도 않고 그냥 거기 있어야 한다.
 */
export const GROUND_SURFACE = {
  /** 안쪽을 채우는 진하기. 순간 표시(0.16)보다 옅다 — 계속 떠 있는 면이라 눈에 남으면 방해다. */
  fillAlpha: 0.10,
  /** 색면 아래에 까는 검은 겹. 밝은 배경 원화 위에서 실루엣을 잡아 준다. */
  backdropAlpha: 0.12,
  /** 테두리선. 어디까지가 물인지는 이 한 줄이 말한다. */
  lineAlpha: 0.30,
  lineWidth: 2,
  /** 들고 날 때 녹아드는 시간(초). 톡 나타나면 그 자체가 깜빡임이 된다. */
  fadeSeconds: 0.45,
  /** 은은하게 숨 쉬는 폭과 주기(초). 물이 살아 있다는 것만 말할 만큼만 움직인다. */
...
export const AREA_IMPACT = {
  ...
  fillAlpha: 0.16,
  /**
   * 색면 아래에 먼저 까는 검은 겹의 진하기.
   *
   * 배경 원화가 밝고 복잡해서(폐허 도로는 거의 흰 콘크리트다) 옅은 색면 한 겹만으로는 통째로
   * 묻힌다 — 특히 보통 피해의 흰빛은 아예 보이지 않는다. 머리 위 체력 바가 아래로 한 겹
   * 복제해 까는 것과 같은 해법이고, 그래야 어느 색이든 어느 바닥 위에서든 같은 무게로 읽힌다.
   */
  backdropAlpha: 0.3,
  /** 테두리 선의 두께와 진하기. */
  lineWidth: 5,
...
/**
 * 전장 전체를 때리는 기술이 켜는 가장자리 워시의 리듬.
 *
 * 마름모와 같은 시간을 쓰되 **차오르고 · 머물고 · 빠지는** 셋으로 나눈다. 마름모처럼 벌어지며
 * 꺼지기만 하면 화면 네 변이 한 번 번쩍이고 마는 것으로 보여, 전장 전체가 맞았다는 뜻이 남지
 * 않는다. 값은 `AREA_IMPACT.ultimateMs`에 곱하는 배수다.
 */
export const BATTLEFIELD_WASH_RISE = 0.45;
export const BATTLEFIELD_WASH_HOLD = 0.5;
