...
interface SkillBase {
   ...
   *
   * 약해진 적을 끝내는 암살자의 마무리다. 판정은 맞기 전의 체력으로 하고, 피해 공식 위에 배율로
   * 얹는다(치명타·방어·상성을 모두 지난 뒤의 몫이 함께 커진다).
   */
  finisher?: {
    /** 표적의 남은 체력이 이 비율(%) 이하면 연다. */
    thresholdPercent: number;
    /** 연 한 방의 피해가 이만큼(%) 늘어난다. 100이면 두 배다. 그 한 방은 확정 치명타이기도 하다. */
    bonusDamagePercent: number;
    /**
     * 한 번 연 표적에게 다시 열기까지 기다리는 시간(초). **표적마다 따로 센다.**
     *
     * 문턱 아래의 모든 타격이 두 배면 마무리가 아니라 상시 강화가 된다. 한 번 크게 터지고, 다른
     * 약해진 적에게는 곧바로 다시 터질 수 있게 한다.
     */
   ...
   */
  /**
   * 이 기술을 쓸 때마다 시전자가 **자기 최대 체력의 이 비율(%)만큼 보호막**을 두른다.
   *
   * 회복이 아니라 보호막인 이유는 레이드 보스 때문이다 — 시즌 게이지는 참가자 전원이 함께 깎은
   * 줄이라 회복은 어제 민 몫을 되돌리지만, 보호막은 그 판 안에서만 남는 한 겹이다. 궁극기에만
...
export type BasicAttack = AttackSkill & {
  ...
  /**
   * 「여울」. 전장 곳곳에 얕은 물을 깔고 **그 사이를 뛰어다니며** 싸운다.
   *
   * 예전에는 판이 **한 곳뿐이고 스피나 발밑에** 고였다. 그러면 여울은 이름만 사냥터일 뿐
   * 사실상 자기 강화 버프였다 — 한 명과 붙어 싸우는 동안 판은 늘 발밑에 있었고, 전장을
   * 일사분란하게 돌아다니는 개체도 아니라 "어디에 고였나"가 아무것도 바꾸지 않았다.
   *
   * 지금은 **적 주위 곳곳에** 물이 고이고, 정해진 타수마다 그중 한 곳으로 순간이동해 그
   * 자리를 친다. 깔고 → 뛰며 터뜨리고 → 궁극기로 전부 터뜨리는 것이 이 개체의 전부다.
   */
  shallows?: {
    /** 판 하나의 반경(px). 판정과 화면이 같은 값을 쓴다. */
    radius: number;
    /** 고인 뒤 물이 마르기까지의 시간(초). */
    seconds: number;
    /**
     ...
     * 아니라 스쳐 지나가기만 해도 걸리는 영구 디버프가 된다. **잠긴 동안에만** 걸린다.
     *
    ...
    /**
     * 고른 적에서 **시전자 반대쪽**으로 이만큼(px) 떨어진 자리에 고인다.
     *
     * 적 발밑에 정확히 고이게 두면 붙어 싸우는 동안 판이 전부 같은 자리에 겹친다. 적을 사이에
     * 두고 건너편에 고이므로, 순간이동이 곧 **적을 지나쳐 반대편으로 파고드는** 걸음이 된다.
     * 자리는 언제나 전장 경계 안으로 조여진다 — 벽에 붙은 적 뒤로는 물이 새지 않는다.
     */
    behindDistance: number;
    /**
     * 새 판이 서려면 기존 판에서 이만큼(px) 떨어져 있어야 한다. 더 가까우면 새로 만들지 않고
     * **그 판의 자리와 시간을 갱신한다.**
     *
     * 이 한 줄이 판의 개수를 정한다 — 상한을 손으로 적지 않아도 적이 몰려 있으면 적게, 흩어져
     * 있으면 많이 깔린다. 숫자로 못 박으면 "왜 네 번째는 안 생기나"를 화면이 말하지 못한다.
     */
    minSpacing: number;
    /** 기본 공격 이 횟수마다 여울 하나로 순간이동한다. */
    leapEveryHits: number;
    /** 내려선 자리에서 그 판 안의 적에게 주는 범위 피해 위력(%). */
    leapPower: number;
    /**
     * 그때 **여울에 잠겨 있던** 적이 더 받는 비율(%).
     *
     * 물을 깔아 둔 자리와 아닌 자리의 차이가 여기 하나로 모인다 — 평타에는 아무 몫도 주지
     * 않으므로, 적을 물가에 가둬 두는 값이 **도약이 꽂히는 순간**에만 돌아온다.
     */
    submergedLeapBonusPercent: number;
  };
} & ({ combo?: undefined } | {
  /**
   * [[combo|연격]]. 한 공격 행동이 확률로 여러 번 들어간다.
   *
   * **회복을 달지 않는다.** 예전에는 각 적중마다 잃은 체력의 5%를 돌려줬는데, 그 한 줄이
   * 암살자에게 전사의 유지력을 쥐여 줘 "낮은 생존력" 대신 붙어 싸울수록 차오르는 몸이
   * 되었다. 물가가 보태는 것은 이빨이지 숨이 아니다(`shallows`).
   */
  combo: {
...
export type Ultimate = Skill & {
  ...
  /**
   * 지휘자 전용: 이 궁극기가 들어간 표적에게 **살아 있는 귀속 소환수가 곧바로 제 궁극기를 쓴다.**
   *
   * 늑대의 게이지는 쓰지 않는다. 무엇을 하는지는 늑대 정의가 갖는다.
   */
  commandsPack?: true;
  /**
  ...
  /**
   * 깔려 있는 [[shallows|여울]]을 **전부 동시에 터뜨린다.** 각 판 자리에 범위 피해가 들어가고
   * 물은 그 자리에서 마른다.
   *
   * 이 궁극기의 값은 한 명을 세게 치는 것이 아니라 **깔아 둔 판을 회수하는 것**이다 — 여울을
   * 몇 곳에 얼마나 잘 벌려 놓았는지가 그대로 피해 총량이 되므로, 평타로 판을 까는 일과
   * 궁극기가 같은 축에 선다. 판이 하나도 없으면 터뜨릴 것도 없다.
   */
  detonateShallows?: {
    /** 판 하나가 터질 때 그 안의 적에게 주는 위력(%). */
    power: number;
  };
  /**
...
export type FerocityTrait = {
  ...
  | {
      ...
      /** 내려선 자리에 여울을 고이게 하는가. 잠행이 곧 사냥터를 여는 걸음이 된다. */
      landingShallows?: true;
...
export interface Passive {
  ...
  durationSeconds?: number;
  /**
   * [[frenzy|광란]]한 적이 **입힌** 피해의 이 비율(%)만큼, 광란을 건 쪽이 회복한다.
   *
   * 제 손으로 때려서 얻는 흡혈이 아니라 **돌려세운 상대가 제 편을 치는 동안** 차오른다.
   * 그래서 뒷줄에 선 마법형이 앞에 나가지 않고도 유지력을 갖고, 그 값은 광란이 실제로
   * 돌고 있는 동안에만 있다 — 판을 뒤집지 못하면 한 점도 차지 않는다.
   *
   * 회복하는 쪽은 개체 이름이 아니라 광란 상태가 들고 있는 `sourceId`가 정한다. 기여도가
   * 같은 값을 읽으므로, 그래프에 잡히는 몫과 빨아들이는 몫이 갈리지 않는다.
   */
  frenzyLifeStealPercent?: number;
  /**
   * 적 하나를 [[taunt|도발]]할 때마다 **잃은 체력**의 일부를 회복한다.
   *
   * 탱커 계약이 요구하는 생존기이되, 「받는 피해 감소」가 아니라 **어그로를 끈 만큼** 돌아오는
   * 값이다 — 데이처럼 달리며 톡톡 치고 빠지는 개체는 막을 두르는 대신 도발한 수가 곧 버틴
   * 양이 된다. 잃은 체력 비례라 가득 찬 몸에는 한 점도 들어오지 않고, 깎일수록 크게 돌아온다.
   *
   * `maxPerSecond`가 없으면 폭주가 주위 전부를 매초 도발하는 순간 한 번에 다섯 배가 들어와
   * 종이 방어의 탱커가 갑자기 죽지 않는 몸이 된다. 시간으로 다시 차는 예산이라 **프레임을
   * 어떻게 쪼개든 초당 횟수가 같다** — 걸린 적 수를 세면 한 호출이 몇 조각으로 나뉘느냐에
   * 따라 회복량이 달라진다.
   */
  tauntHeal?: {
    /** 한 번 발동할 때 회복하는, 지금 잃은 체력의 비율(%). */
    missingHpPercent: number;
    /** 1초 동안 발동할 수 있는 최대 횟수. */
    maxPerSecond: number;
  };
  /**
   * 전투가 열리는 순간부터 이 시간(초)만큼 [[stealth|은신]]한 채로 시작한다.
   *
   * **패시브 종류가 아니라 이 필드 하나가** 여는 은신을 정한다 — `kind`로 갈랐을 때는 같은
   * 시작 은신을 갖고 싶은 다음 개체마다 전용 종류를 하나씩 더 만들어야 했다. 델로피의
   ...
   *
   * **패시브 종류가 아니라 이 계약이** 경계와 시간을 정한다 — `kind`에 못 박아 두었을 때는
   * 같은 위기 은신을 다른 경계로 쓰고 싶은 개체마다 전용 종류를 하나씩 더 만들어야 했다.
   * 스테라의 「선망 받는 루키의 일상」과 파루아의 「나무가 아닌 숲을!」이 서로 다른 패시브
   * 이면서 같은 필드를 읽는다.
   */
  lowHpStealth?: {
    /** 이 비율(%) 이하가 되는 순간 발동한다. 정확히 그 값인 순간도 포함한다. */
    hpPercent: number;
    /** 은신이 유지되는 시간(초). */
    seconds: number;
    /**
     * 발동하면서 쌓아 둔 [[focus|집중]]을 모두 잃는가.
     *
     * **숨는 값을 공짜로 주지 않기 위한 대가다.** 파루아는 쏠수록 공격력과 사거리가 함께
     * 자라는 개체라, 위기에 숨는 것이 곧 "다시 처음부터 쌓는다"가 되어야 그 은신이 도망이지
     * 보상이 아니게 된다.
     */
    spendsFocus?: true;
  };
...
export interface SummonScaling {
  /**
   * 체력 파생 비율이다. **주인의 공격력 + 주문력**에 곱한다.
   *
   * 몸(체력·방어·저항)은 두 축의 합을 따른다 — 한 축만 따르면 주인이 다른 축을 키울 때 앞에 선
   * 방패가 그대로 남는다. 때리는 손(공격력·주문력)과 속도만 제 성장 축(`growthStat`)을 따른다.
   */
  hp: number;
  /** 공격력 파생 비율이다. 성장 축(`growthStat`) 한 값에 곱한다. */
  atk: number;
  /** 방어력 파생 비율이다. 체력과 같이 주인의 공격력 + 주문력에 곱한다. */
  def: number;
  /** 저항력 파생 비율이다. 체력과 같이 주인의 공격력 + 주문력에 곱한다. */
  res: number;
  /** 공격 속도 파생 비율이다. */
  attackSpeed: number;
  /** 이동 속도 파생 비율이다. */
...
export interface BattleStageDef extends StageBase {
   ...
   * **단일 정예 관문**이라는 표식.
   *
   * 원정 지도의 정예 노드처럼 여럿 대신 하나가 서고, 그 하나가 더 무겁다. 무거워지는 몫은
   * **유형 표**(`ENCOUNTER_ROLE.elite`)가 갖는다 — 셋이 나눠 내던 체력을 하나가 대신하는
