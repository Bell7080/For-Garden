...
export class TradePopup {
  ...
  open(): void {
    ...
    this.body = this.popups.open({ width: 940, height: 1420, title: "무역", dim: true, closeOnBackdrop: false, hideCloseButton: true, onClose: () => this.dispose() }, (_body, close) => { this.closeAction = close; });
    void this.refresh();
  }
  ...
  private async refresh(): Promise<void> {
    ...
    if (!this.body || generation !== this.generation) return;
  ...
  private render(): void {
    ...
    this.products.forEach((product, index) => {
      ...
      this.body?.add(this.scene.add.text(-360, y, product.name, textStyle({ role: "emphasis", size: 29 })).setOrigin(0, 0.5));
      this.body?.add(this.scene.add.text(-360, y + 48, action.priceText, textStyle({ role: "body", size: 22, color: COLOR.inkDim })).setOrigin(0, 0.5));
      const button = new Button(this.scene, 245, y + 20, { width: 270, height: 76, label: action.label, onClick: () => this.openPurchase(product) });
      button.setEnabled(!action.disabledReason); this.body?.add(button);
      if (action.disabledReason) this.body?.add(this.scene.add.text(370, y + 70, action.disabledReason, textStyle({ role: "body", size: 18, color: COLOR.inkDim })).setOrigin(1, 0.5));
    });
  }
  ...
  private dispose(): void { this.generation += 1; this.closeAction = undefined; this.body = undefined; this.onClosed?.(); }
}
