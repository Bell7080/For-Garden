...
- **로비**는 캐릭터를 보는 화면이다. 조작은 가장자리로 밀고 가운데는 비운다.
  왼쪽 위 = 플레이어 프로필(`TopBar`), 오른쪽 위 = 재화와 설정, 그 아래 = 홍보 칸,
  왼쪽 중단 = 상점·우편·친구 아이콘 줄과 교류 입구(`RailButton`),
  오른쪽 아래 = 원정과 출격. 강조 버튼(`variant: "primary"`)은 화면에 하나만 둔다.
- 로비 대사는 말풍선 판을 씌우지 않는다. 화면 중단에 금빛 이름과 큰 글자를 얹고 위로
  떠오르며 사라진다. 판이 아니라 크기와 옅은 유리면이 읽히는 힘을 담당한다.
...
- 아래에는 **하단 레이어**를 겹쳐 `LV.n`(강조색)과 이름을 얹는다. 둘 사이에 구분 기호를 두지
  않고 색으로만 가른다. 하단 레이어는 칩보다 넓어 보이되 **카드 폭을 넘지 않는다** — 넘으면
  옆 칸과 겹친다. 넓히는 대신 칩을 `CHIP_INSET`만큼 좁힌다.
