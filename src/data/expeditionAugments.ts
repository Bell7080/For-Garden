...
export const EXPEDITION_AUGMENTS = [
  ...
  { id: "reinforced-core", name: "강화 코어", rarity: "sr", target: "party", category: "attack", effect: { kind: "attackPowerPercent", percent: 8 } },
  // 공격/SR: 한 기에 집중하는 대신 전체 효과의 약 두 배인 18%를 준다.
  { id: "predator-instinct", name: "포식 본능", rarity: "sr", target: "relic", category: "attack", effect: { kind: "attackPowerPercent", percent: 18 } },
  // 주문/SR: 마법 조합의 전체 주문력을 공격력 전체안과 같은 8%로 맞춘다.
  { id: "echo-circuit", name: "공명 회로", rarity: "sr", target: "party", category: "spell", effect: { kind: "spellPowerPercent", percent: 8 } },
  // 주문/SR: 개인 주문력 18%는 물리 개인안과 같은 단일 대상 예산이다.
  { id: "focused-spectrum", name: "집속 스펙트럼", rarity: "sr", target: "relic", category: "spell", effect: { kind: "spellPowerPercent", percent: 18 } },
  // 생존/SR: HP는 회복·보호막과 시너지가 있어 전체 예산을 7%로 한 단계 낮춘다.
  { id: "vital-lattice", name: "생체 격자", rarity: "sr", target: "party", category: "survival", effect: { kind: "maxHpPercent", percent: 7 } },
  // 생존/SR: 물리 방어만 올리므로 전체 10%까지 허용한다.
  { id: "layered-carapace", name: "적층 갑각", rarity: "sr", target: "party", category: "survival", effect: { kind: "defensePercent", percent: 10 } },
  ...
  { id: "relentless-hunt", name: "불굴의 추적", rarity: "ssr", target: "relic", category: "attack", effect: { kind: "attackPowerPercent", percent: 28 } },
  // 공격/SSR: 행동 횟수는 에너지·상태 발동도 늘리므로 전체 공속을 보수적인 14%로 둔다.
  { id: "overclock-field", name: "과회전 역장", rarity: "ssr", target: "party", category: "attack", effect: { kind: "attackSpeedPercent", percent: 14 } },
  // 주문/SSR: 전체 주문력 16%는 물리 정점 신호와 대칭이다.
  { id: "astral-resonance", name: "성운 공명", rarity: "ssr", target: "party", category: "spell", effect: { kind: "spellPowerPercent", percent: 16 } },
  // 생존/SSR: 최대 HP 전체 14%는 보호막·회복 시너지 때문에 공격 예산보다 낮다.
  { id: "primeval-vigor", name: "태고의 활력", rarity: "ssr", target: "party", category: "survival", effect: { kind: "maxHpPercent", percent: 14 } },
  // 생존/SSR: 두 피해 경로를 동시에 올리지 않고 개인 방어에 30%를 집중한다.
  { id: "adamant-shell", name: "불괴의 외피", rarity: "ssr", target: "relic", category: "survival", effect: { kind: "defensePercent", percent: 30 } },
