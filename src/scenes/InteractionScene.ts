...
export class InteractionScene extends Phaser.Scene {
  ...
  private draw(rows = INTERACTION_CITIES.map(city => ({ ...city, unlocked: city.unlock.researchLevel <= session.playerResearch.level }))): void {
    ...
    add(this.add.text(78, 555, `추천 ${city.recommended.elements.join(" · ")} / ${city.recommended.squads.join(" · ")}`, textStyle({ role: "body", size: 22, color: COLOR.inkDim }))); add(this.add.text(78, 610, `보상 성향  ${city.rewards.map(r => `${r.currency} ${r.amount}`).join(" · ")}`, textStyle({ role: "body", size: 23 })));
