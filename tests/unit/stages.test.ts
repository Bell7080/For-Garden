...
describe("stage enemy design", () => {
  ...
  it("챕터 1의 레벨·돌파 초안을 적별 StageEnemyDef에 정확히 기록한다", () => {
     ...
     * 값은 `stageBalance.ts`의 성장 곡선에서 풀었다 — 관문마다 바닥 파티의 전멸선에 얼마나
     * 다가서는가(`stageApproachRatio`)로 레벨을 정하고, 그 선은 실제 전투를 돌려 찾는다.
     * v0.97.0에서 성장이 주능력치 다섯만 올리게 되어 전멸선이 함께 움직였으므로 같은 절차로
     * 다시 풀었다.
     */
