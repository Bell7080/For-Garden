...
describe("토리카 스킬 표시 계약", () => {
  it("은 구조화된 회복·실제 방어 수치·주위 도발을 실제 표시 문구로 만든다", () => {
    ...
    expect(torika.passive).toMatchObject({ value: 3.5, durationSeconds: 5 });
    expect(`${torika.passive.durationSeconds}초 동안 ${recoveryLabel(torika.passive.value)}`).toBe("5초 동안 매초 최대 체력의 3.5% 회복");
    // **규칙어로 감싸지 않는다.** 「지속 회복」은 "정해진 시간 동안 일정한 간격으로"까지만 말해,
    // 정작 이 패시브가 말해야 하는 시간과 몫이 눌러도 나오지 않았다. 문장이 직접 적는다.
    expect(torika.passive.desc).toBe("전투당 한 번, 체력이 절반 이하가 되면 5초 동안 매초 최대 체력의 3.5%를 회복한다.");
    expect(torika.passive.desc).not.toContain("[[regeneration");
...
describe("데이 스킬 표시 계약", () => {
  ...
  it("의 밴덜리즘은 태그가 수치를 갖고 본문은 한 겹만 말한다", () => {
    ...
    expect(effect).toMatchObject({ offenseShredPercent: 5, maxStacks: 5, burstPower: 125 });
    // 시간으로 사라지지 않는다는 것이 이 규칙어의 전부라, 태그가 그 사실을 직접 말한다.
    expect(effect).not.toHaveProperty("seconds");
    // 태그 본문의 수치는 실제 전투가 읽는 필드와 같아야 한다 — 갈리면 조정 뒤 옛 문장이 남는다.
    expect(keyword.description).toContain(`${effect.kind === "vandalism" ? effect.offenseShredPercent : 0}% 낮아지며`);
    expect(keyword.description).toContain("시간이 흘러 사라지지 않는다");
    expect(keyword.description).toContain("5겹째");
    expect(keyword.description).toContain("125%");
  ...
  it("의 궁극기는 총량이 아니라 매초의 몫으로 읽힌다", () => {
    ...
    expect(skillDescription(deina.ultimate, { damage: 17 }))
      .toBe("전장의 모든 적에게 5초 동안 매초 [[damage-value|17]]의 [[magical-damage|마법 피해]]를 주고 [[vandalism|밴덜리즘]]을 한 겹 쌓는다."
  ...
  it("는 탱커이면서 딜러 화력을 내지 않는다", () => {
    ...
    // 초안이 주문력 152·평타 70%였고 표준 5인 파티 재현에서 적 체력의 30%를 혼자 깎아
    // 렉시아(22%)·스피나(28%)를 넘었다. 지금은 16%로 마키(17%) 자리다. 되돌리면 여기서 걸린다.
...
describe("폰토스 스킬 표시 계약", () => {
  it("의 패시브는 복리 누적률·경감 띠·무효화 문턱·강인함을 모두 명시한다", () => {
    const pontos = RELICS.find((def) => def.id === "pontos")!;
    /*
     * 수치는 그대로 두되 문장을 줄였다. "완전히 경과한" · "선형으로 증가하며" · "그 이하에서는
     * 최대치로 제한된다"는 구현을 설명하는 말이라 플레이어의 조작을 바꾸지 않는다. 능력치
     * 이름은 규칙어 태그로 걸어 눌러서 열 수 있게 한다.
     *
     * **경감이 상한에 닿는 체력도 적지 않는다.** 그 자리가 체력이 다 닳는 지점이라
     * "최대의 0%까지 낮아질수록"이 되어 말이 되지 않는다 — 곡선이므로 문장은 어디서 시작해
     * 어디까지 가는지만 말한다.
     */
    expect(passiveDescription(pontos.passive)).toBe(
      "매초 [[ap|주문력]]이 2%씩 복리로 오른다."
      + " [[hp|체력]]이 낮아질수록 받는 모든 피해가 50%에서 99%까지 줄고, 10 이하의 피해는 무효가 된다."
      + " [[crowd-control|군중제어]]에 걸릴 때마다 강인함이 8% 올라 100%까지 쌓이고, 그만큼 제어 시간이 줄어든다.",
    );
...
describe("스피나 스킬 표시 계약", () => {
  it("은 네 슬롯의 이름·요약·구조화 수치를 정적 정의와 함께 유지한다", () => {
    ...
    expect(ferocityTraitDescription(spino.ferocityTrait)).toContain("그 적 주위에 [[shallows|여울]]이 고인다");
    ...
    expect(skillDescription(spino.basic, { damage: 100 })).toBe(
      ...
      /*
       * **두 규칙어가 나눠 맡는다.** 물이 무엇인지는 「여울」이, 언제 어떻게 뛰는지는 「강하」가
       * 말한다 — 한 태그에 둘을 담았을 때는 다른 규칙어의 두세 배 길이가 되면서 정작 둘 다
