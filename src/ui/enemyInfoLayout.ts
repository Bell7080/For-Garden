...
export const ENEMY_INFO = {
  ...
  /**
   * 이름과 두 뱃지가 끝나야 하는 오른쪽 선 — 돌파 등급 표식의 왼쪽 가장자리 앞이다.
   *
   * 이름이 긴 개체(수쿠스이노)는 뱃지를 오른쪽 기둥 위까지 밀어내 레벨 칸의 제목표 밑으로
   * 들어가게 했다. 이름 글자를 이 선 안으로 줄이고(`shrinkTextToWidth`), 레벨 칸은 그 줄
   * 아래에서 시작한다(`levelPanel.top`).
   */
  nameRight: 246,
  /**
  ...
  /**
   * 레벨 칸 — 읽기 전용이라 경험치·급여가 빠진 정보창의 축소 판이다.
   *
   * **제목표가 이름 줄 아래에서 시작한다.** 이름 줄과 같은 높이에 걸터앉던 때는 뱃지가 오른쪽
   * 기둥까지 밀린 개체에서 `/레벨` 표가 속성·직군을 덮었다.
   */
  levelPanel: { top: -350, height: 140 },
  /** 능력치 칸. 오각형 반지름·사거리 줄·돋보기 자리가 모두 정보창과 같은 간격이다. */
  statPanel: { top: -162, height: 446 },
   ...
   * 전신 원화.
   *
   * 정보창과 같이 **원화 그대로의 크기로 세워 다리께에서 판 밑변에 잘린다** — 상자에 맞춰
   * 줄이면 얼굴이 작아져 누구인지보다 여백이 먼저 읽힌다. 코어(`중심1`) 관절을 기준으로 세우고
   * 판과 같은 실루엣으로 잘라 깎인 모서리 밖으로 나가지 않게 한다.
   *
   * 배율을 키우면 **코어에서 머리끝까지의 거리도 함께 늘어나** 정수리가 판 윗변으로 밀려
   * 올라간다. 그래서 키운 만큼 `coreY`를 조금 내려 얼굴이 이름 블록 아래 제자리에 남게 한다.
   */
  portrait: { x: -192, coreY: -10, height: 1400 },
  /**
   * 원화를 통째로 보는 돋보기.
   *
   * **적에게도 둔다.** 아군 정보창에는 있고 적 창에만 없으면, 같은 파이프라인으로 구운 같은
   * 크기의 원화인데 한쪽만 자세히 볼 수 있다. 자리는 아군 창과 같은 결로 **이름 블록 아래
   * 왼쪽**이다 — 원화 위에 서지만 판 왼쪽 끝이라 인물의 얼굴을 덮지 않는다.
   */
  portraitMagnifier: { x: -366, y: -196 },
  /**
   * 관찰 일지 입구 — 돋보기 바로 위, 같은 기둥에 선다. 아군 정보창과 같은 순서(일지 → 돋보기)다.
   *
   * **적에게도 둔다.** 적 개체도 개체번호·프로젝트·발굴지·복원 후 관찰 기록과 소속 스쿼드를
   * 제 정의에 온전히 갖고 있는데(아군과 같은 `RelicDef` 정체성 규칙), 그것을 여는 문이 아군
   * 정보창에만 있어 **화면 어디에서도 읽을 수 없었다** — 기껏 적어 둔 설정과 소속 엠블럼이
   * 데이터에만 남아 있는 셈이었다.
   *
   * 자리는 원화 돋보기와 같은 x다. 둘 다 왼쪽 끝이라 인물의 얼굴을 덮지 않고, 세로로 붙여
   * 두면 "이 개체를 더 들여다보는 조작"이 한 덩어리로 읽힌다.
   */
  journalButton: { x: -366, y: -300 },
