...
/** 지정 질문이 순환 중 등장하는 날짜를 찾아 테스트가 배치 구현 세부값에 묶이지 않게 한다. */
function dateForQuestion(questionId: string): string {
  for (let day = 1; day <= 31; day += 1) {
    const date = `2026-08-${String(day).padStart(2, "0")}`;
    if (observationQuestionForRelicAndDate("anky", date).id === questionId) return date;
  }
...
describe("관찰 인터뷰", () => {
  it("같은 토리카와 UTC 날짜에는 항상 같은 전용 질문을 조합한다", () => {
    ...
    const date = dateForQuestion(questionId);
  ...
  it("각 신규 선택의 서로 다른 태그와 습성 문장을 기록한다", () => {
    for (const question of RELIC_OBSERVATION_QUESTIONS.anky) {
      const date = dateForQuestion(question.id);
  ...
  it("과거 공용 질문 ID와 날짜 전용 API의 결과를 그대로 유지한다", () => {
    ...
    const allIds = [...COMMON_OBSERVATION_QUESTIONS, ...RELIC_OBSERVATION_QUESTIONS.anky].map(({ id }) => id);
