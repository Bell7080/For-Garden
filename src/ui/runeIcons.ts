...
import { RUNE_CHANCE_ROLL, RUNE_MARK, type RuneCraftImpactKind } from "./runeCraftMotion";
import { runePieceFit } from "./runePieceContent";
import { STAT_TONE } from "./statTones";
import { addGlowStar } from "./stars";
...
export function addEmptyRuneMark(scene: Phaser.Scene, parent: Phaser.GameObjects.Container, x: number, y: number, outer: number): Phaser.GameObjects.Container {
  const mark = scene.add.container(x, y);
  mark.add(scene.add.star(0, 0, 4, outer * 0.34, outer, 0x000000, 0.32).setStrokeStyle(2, 0x8a929c, 0.34));
  parent.add(mark);
  return mark;
}
 ...
 *
 * **컨테이너로 돌려준다.** 결과는 그려지는 것이 아니라 **박히는 것**이라, 부른 쪽이 이 한
 * 덩어리를 크게 나타냈다가 제 크기로 앉히는 동작을 걸 수 있어야 한다. 겹마다 tween을 걸면
 * 다섯 겹이 따로 움직여 표식이 벌어진다.
 */
export function addRuneMark(
  ...
  y: number,
  outer: number,
  kind: RuneCraftImpactKind,
): Phaser.GameObjects.Container {
  // 각인도 **같은 다이아**다. 모양까지 바꾸면 한 줄 안에서 다른 종류의 표식으로 읽히고, 크게
  // 키우면 그 칸 하나가 룬 전체보다 먼저 눈에 들어온다. 완성은 색(노란 발광)이 말한다.
  const mark = scene.add.container(x, y);
  addGlowStar(scene, mark, 0, 0, outer, RUNE_MARK[kind], 4);
  parent.add(mark);
  return mark;
}

/** 확률 줄 한 장이 받는 값. 판이 커지면 글자도 함께 커진다. */
export interface RuneChanceLineOptions {
  /** 수치 글자 크기. */
  labelSize: number;
  /** 수치 글자 아래끝이 막대 위로 얼마나 떨어져 있는지. */
  labelBaselineY: number;
  /** 수치 글자가 줄 양 끝에서 안으로 들어오는 거리. */
  labelInset: number;
  /** 처음 보여 줄 확률. 다시 그려도 직전에 보이던 값에서 이어지도록 부른 쪽이 들고 있는다. */
  from: number;
  /** 보이는 값이 바뀔 때마다 알린다. 다시 그릴 때 이어 가려고 부른 쪽이 기억한다. */
  onRoll?: (shown: number) => void;
}

/**
 * 성공과 실패가 한 줄을 나눠 가지는 발광 막대와, 그 위의 두 수치.
 *
 ...
 *
 * **막대와 두 수치를 한 장이 함께 갖는다.** 따로 두면 막대는 흘러가는데 숫자만 갈아 끼워져,
 * 흘러가는 동안 둘이 다른 값을 말한다. 수치는 아래로 한 겹 복제한 검은 그림자를 달아 밝은
 * 배경 원화 위에서도 읽히게 한다 — 판을 받치면 확률 줄 위에 상자가 하나 더 생긴다.
 */
export class RuneChanceLine {
  private static readonly HEIGHT = 18;
  private readonly graphics: Phaser.GameObjects.Graphics;
  private readonly successLabel: readonly Phaser.GameObjects.Text[];
  private readonly failLabel: readonly Phaser.GameObjects.Text[];
  private readonly state: { shown: number };
  ...
  constructor(
    ...
    private readonly options: RuneChanceLineOptions,
  ) {
    this.state = { shown: clampChance(options.from) };
    const label = (align: 0 | 1, color: string): readonly Phaser.GameObjects.Text[] => {
      const at = align === 0 ? x - width / 2 + options.labelInset : x + width / 2 - options.labelInset;
      const style = textStyle({ role: "emphasis", size: options.labelSize, color });
      const shadow = scene.add.text(at + 3, y - options.labelBaselineY + 4, "", textStyle({ role: "emphasis", size: options.labelSize, color: "#05070a" })).setOrigin(align, 1).setAlpha(0.85);
      const front = scene.add.text(at, y - options.labelBaselineY, "", style).setOrigin(align, 1);
      parent.add([shadow, front]);
      return [shadow, front];
    };
    this.successLabel = label(0, `#${RUNE_MARK.success.halo.toString(16).padStart(6, "0")}`);
    this.failLabel = label(1, `#${RUNE_MARK.fail.glow.toString(16).padStart(6, "0")}`);
    this.graphics = scene.add.graphics();
    parent.add(this.graphics);
    // 판이 다시 그려지면 그림도 함께 죽는다. 흘러가던 tween을 함께 세우지 않으면 죽은
    // 그림에 계속 그리려 들어 한 프레임마다 조용히 실패한다.
    this.graphics.once(Phaser.GameObjects.Events.DESTROY, () => this.tween?.stop());
    this.paint();
  }
   ...
   * 목표 확률로 흘러간다.
   *
   * 곧바로 갈아 끼우면 10%씩 움직인 것이 화면에 남지 않는다. 연타로 다음 결과가 먼저 오면
   * **지금 보이던 값에서 이어** 흘러가므로, 굴러가다 끊겨도 값이 튀지 않는다.
   */
  rollTo(target: number, animate: boolean): void {
    const to = clampChance(target);
    this.tween?.stop();
    this.tween = undefined;
    if (!animate || Math.abs(to - this.state.shown) < 0.001) {
      this.state.shown = to;
      this.paint();
      this.options.onRoll?.(to);
      return;
    }
    this.tween = this.scene.tweens.add({
      targets: this.state,
      shown: to,
      duration: RUNE_CHANCE_ROLL.ms,
      ease: RUNE_CHANCE_ROLL.ease,
      onUpdate: () => { this.paint(); this.options.onRoll?.(this.state.shown); },
      onComplete: () => { this.tween = undefined; this.options.onRoll?.(to); },
    });
  }

  /** 지금 보이는 값. 판을 다시 그릴 때 이 값에서 이어 간다. */
  get shown(): number {
    return this.state.shown;
  }

  private paint(): void {
    const clamped = clampChance(this.state.shown);
    const { x, y, width } = this;
    const height = RuneChanceLine.HEIGHT;
    ...
    const glow = (flat: number[], color: number): void => {
      const points = toGeomPoints(flat).map((point) => new Phaser.Geom.Point(point.x, y + (point.y - y) * 1.9));
    ...
    graphics.fillPoints(toGeomPoints(success), true);
    graphics.fillStyle(RUNE_MARK.fail.body, 1);
    graphics.fillPoints(toGeomPoints(fail), true);
...
/** 확률은 0~1 밖으로 나가지 않는다. 저장이 손상되어도 막대가 판을 넘지 않게 한다. */
