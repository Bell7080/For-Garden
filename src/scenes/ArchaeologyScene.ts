...
export class ArchaeologyScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    panel.add(this.add.text(0, -110, t("archaeology.unknownZone"), textStyle({ role: "display", size: 40, color: COLOR.accentText })).setOrigin(0.5));
    panel.add(this.add.text(0, 0, t("archaeology.zoneNote"), textStyle({ role: "body", size: 29, color: COLOR.inkDim, align: "center" })).setOrigin(0.5));
    panel.add(this.add.text(0, 145, t("archaeology.comingSoon"), textStyle({ role: "emphasis", size: 27, color: COLOR.ink })).setOrigin(0.5));
