...
export const INFO_KO = {
  ...
  // 적 정보창의 역할 칸 — 이 적이 어떤 자리로 섰는가. 수치는 유형 표(`ENCOUNTER_ROLE`)에서 온다.
  "info.enemy.role": "역할",
  ...
  "info.enemy.role.lead.normal": "셋이 함께 서는 기본 자리다. 레벨로만 자란다.",
  "info.enemy.role.lead.swarm": "다섯이 한꺼번에 몰려오는 자리다. 하나하나는 가볍게 서는 대신 수가 많다.",
  "info.enemy.role.lead.elite": "혼자 서서 셋 몫을 하는 자리다. 한 번에 하나만 때리므로 버티는 몫이 때리는 몫보다 크다.",
  "info.enemy.role.lead.boss": "여럿이 함께 미는 자리다. 판 안에서는 쓰러지지 않고, 체력은 모두가 함께 깎는 한 줄이 정한다.",
  "info.enemy.role.lead.endless": "쓰러지지 않는 자리다. 눕히는 것이 아니라 제한 시간 안에 얼마나 밀었는지를 잰다.",
  "info.enemy.role.stat.hp": "체력 ×{value}",
  "info.enemy.role.stat.attack": "공격력·주문력 ×{value}",
  "info.enemy.role.stat.body": "몸집 ×{value}",
  "info.enemy.role.stat.none": "추가 배율 없음",
  "info.enemy.role.tenacity": "[[tenacity|강인함]] {base}%로 시작하고, [[crowd-control|군중제어]]를 받아 낼 때마다 {per}%씩 올라 {max}%까지 쌓인다.",
  "info.enemy.role.damageReduction": "[[damage-reduction|경감]]이 {base}%에서 시작해 [[hp|체력]]이 깎일수록 {max}%까지 커지고, 경감을 지난 피해가 {ignore} 이하이면 들어가지 않는다.",
  "info.enemy.role.included": "배율은 능력치에 이미 들어 있다.",
  "info.section.rune": "룬",
  "info.level": "레벨",
