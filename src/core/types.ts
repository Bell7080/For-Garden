...
export type BasicAttack = AttackSkill & {
  ...
  shallows?: {
    ...
    /**
     * 판에 잠긴 적을 때릴 때 연격이 **몇 번** 들어가는가. 물 밖의 확률 연격(`combo.hitCount`)
     * 보다 크게 둔다.
     *
     * 여울을 "평타가 한 번 더 들어가는 장판"에서 **이 개체가 화력을 내는 자리**로 옮기는 값이다.
     * 물 밖에서는 40%로 두 대, 물가에서는 확정으로 이 횟수 — 그래서 사냥감을 물가에 가두는
     * 것 자체가 곧 다단히트가 되고, 공격 속도 누적(「전투의 환희」)이 그 위에서 곱해진다.
     */
    submergedHitCount: number;
    /**
     * **물이 메워 준 대**가 내는 위력(%). 제 힘으로 문 대(확률 연격)는 그대로 100%다.
     *
     * 다단히트를 **타수로만** 늘리면 화력이 타수에 비례해 그대로 곱해진다 — 네 대로 두었을 때
     * 11레벨 스피나 한 명이 혼자 2장을 끝냈다(`하이퍼 캐리 혼자서는 2장을 끝내지 못한다`).
     * 편성 칸이 셋인 이유를 지우는 값이라, 늘어난 타수는 **더 자주 물되 한 입이 얕게** 문다.
     *
     * 그래서 이 개체의 여울은 "피해를 두 배로 만드는 장판"이 아니라 **물어뜯는 횟수를 늘리는
     * 자리**다 — 화면에서는 숫자가 네 번 튀고, 총합은 그보다 훨씬 덜 오른다.
     */
    submergedExtraHitPowerPercent: number;
    /**
     * 판에 잠긴 적에게 주는 피해가 오르는 비율(%).
     *
     * 연격에서 걷어 낸 회복을 대신하는 값이고, 그 자리를 **피해**로 돌려준 이유는 암살자의
     * 값어치가 순간 화력이기 때문이다. 무엇보다 이 몫은 **자리에 걸려 있다** — 달려가는
     * 동안과 물이 마른 뒤에는 한 점도 오르지 않으므로, "물가로 끌어들여 잡는다"가 상시
     * 배율이 아니라 지금 어디에 서 있느냐로 읽힌다.
     *
     * 방어·상성과 같은 최종 피해 경계에서 한 번만 곱한다.
     */
    submergedDamagePercent: number;
  };
} & ({ combo?: undefined } | {
  /**
...
export type Ultimate = Skill & {
  ...
  /**
   * 시전한 자리에 **범람한 여울**을 남긴다. 기본 공격이 고이는 판과 같은 슬롯을 덮어쓴다.
   *
   * 평타가 발밑에 찔끔 고이는 것과 달리 이쪽은 한 번에 넓고 오래 간다 — 궁극기가 "한 명을
   * 세게 친다"에서 **"사냥터를 연다"**로 바뀌는 자리이고, 그 뒤의 평타가 전부 그 위에서
   * 다단히트로 들어간다. 반경은 기본 판의 배수로 적어, 평타 여울을 고치면 범람도 함께 움직인다.
   */
  floodShallows?: {
    /** 기본 여울 반경에 곱할 배수. */
