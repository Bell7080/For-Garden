...
interface SkillBase {
   ...
   * 「반짝!」 이 타격이 표식을 다루는 방식.
   *
   * **표식은 맞은 쪽이 든다.** 공격자가 하나만 들고 다니던 때는 여럿을 함께 때리는 타격이
   * 표식을 어디에 남기는지 말할 수 없었다. 규칙은 어느 타격에서나 같다 — 표식이 없는 적을
   * 때리면 표식을 남기며 추가 마법 피해를 주고, 표식이 있는 적을 때리면 그 표식이 사라진다.
   * 사라지는 순간에 무엇이 터지는지만 스킬이 정한다.
   */
  shimmer?: {
    /** 표식을 새로 남길 때 함께 주는 추가 마법 피해의 주문력 계수(%). */
    markPower: number;
    /** 표식이 사라질 때 그 자리에서 터지는 범위 피해의 주문력 계수(%). 없으면 지우기만 한다. */
    burstPower?: number;
    /** 그 범위 피해가 번지는 반경(px). */
    burstRadius?: number;
    /** 터진 피해의 이 비율(%)만큼 시전자가 보호막을 얻는다. */
    burstShieldPercent?: number;
  };
  /**
...
export type PassiveKind =
  ...
  | "gourmetHunt"
  /** 티아 전용: 스스로 매초 체력을 되찾고 공격 속도를 올린다. */
  | "tidalVigor"
...
export interface Passive {
   ...
   */
  lifeStealPoints?: number;
  /** 스스로 매초 되찾는 최대 체력 비율(%). 전투 내내 도는 자가 재생이라 시간이 없다. */
  maxHpRegenPercentPerSecond?: number;
  /**
   * 전투가 열리는 순간 **멀리 선 표적에게 파고든다.**
   *
   * 개체 이름이 아니라 표적의 `reachTier`로 가른다 — 붙어 서는 개체에게까지 달려들면 이미
   * 사거리 안이라 아무 일도 일어나지 않고, 왜 어떤 판에서만 달려드는지 화면이 말하지 못한다.
   */
  openingCharge?: {
    /** 이 사거리 등급의 표적에게만 파고든다. */
    againstReachTiers: readonly ReachTier[];
