...
import { bountyRoundEnemy, BOUNTY_PRESENCE, BOUNTY_TIERS, getBountyTier } from "./bounty";
import { CAKE_OPERATION_TIERS, cakeOperationEnemies, cakeOperationPresence, getCakeOperationTier } from "./cakeOperation";
...
import { getStageEnemies, stageEnemyGrowth } from "./stages";
...
export function partyPreview(content: PartyContent, stage: BattleStageDef): PartyPreview {
  if (content.content === "raid") {
    ...
    const shown = [{ def, level: RAID_SEASON_BOSS.level, breakthrough: RAID_SEASON_BOSS.breakthrough, ferocityLevel: RAID_SEASON_BOSS.ferocityLevel }];
    return { shown, all: [def], presence: "raid" };
  }
  if (content.content === "bounty") {
    const shown = getBountyTier(content.tierId).rounds.map((round, index) => ({
      def: bountyRoundEnemy(round), level: round.level, breakthrough: 0,
      ...(round.ferocityLevel ? { ferocityLevel: round.ferocityLevel } : {}), round: index + 1,
    }));
    return { shown, all: shown.map(({ def }) => def), presence: BOUNTY_PRESENCE };
  }
  if (content.content === "cake") {
    ...
    const shown = all.slice(0, 5).map((def) => ({ def, level: tier.enemyLevel, breakthrough: 0, ...(tier.ferocityLevel ? { ferocityLevel: tier.ferocityLevel } : {}) }));
    return { shown, all, presence: cakeOperationPresence(tier), hordeCount: all.length };
  }
  ...
  const shown = all.map((def, slot) => ({
    ...
    ...(growth[slot]?.ferocityLevel ? { ferocityLevel: growth[slot].ferocityLevel } : {}),
  }));
  return { shown, all, presence: enemyPresenceFor(all.length, { elite: stage.elite === true }) };
}
