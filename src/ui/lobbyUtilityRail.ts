...
export function createLobbyUtilityRail(actions: LobbyUtilityRailActions) {
  ...
  return [
    { bounds: LOBBY_RAIL_BOUNDS.utility.mail, icon: "mail", label: "우편", onClick: actions.openMail },
    { bounds: LOBBY_RAIL_BOUNDS.utility.friends, icon: "friends", label: "친구", onClick: actions.openFriends },
    { bounds: LOBBY_RAIL_BOUNDS.utility.inventory, icon: UI_ICON.bag, label: "가방", onClick: actions.openInventory },
  ] as const;
}
