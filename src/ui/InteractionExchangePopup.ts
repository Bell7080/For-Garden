...
export class InteractionExchangePopup {
  ...
  open(): void { this.popups.open({ width: 880, height: 1040, title: t("interaction.exchange"), dim: true, closeOnBackdrop: true, backButton: true }, (body) => { this.content = this.scene.add.container(0, 20); body.add(this.content); void this.refresh(); }); }
  ...
  private paintRow(content: Phaser.GameObjects.Container, offer: InteractionExchangeOfferDto, index: number, result: string): void { const y = -330 + index * 330; const quantity = Math.min(this.quantities.get(offer.id) ?? 1, Math.max(1, offer.remaining)); this.quantities.set(offer.id, quantity); const reward = offer.grants[0]; const rewardAmount = reward ? reward.amount * quantity : 0;
    content.add(this.scene.add.text(-350, y, t("interaction.exchange.owned", { owned: offer.cost.owned }), textStyle({ role: "emphasis", size: 28 })).setOrigin(0, .5));
    content.add(this.scene.add.text(-350, y + 58, t("interaction.exchange.required", { amount: offer.cost.amount * quantity }), textStyle({ role: "body", size: 25, color: COLOR.inkDim })).setOrigin(0, .5));
    content.add(this.scene.add.text(-350, y + 116, t("interaction.exchange.result", { amount: rewardAmount }), textStyle({ role: "body", size: 25, color: COLOR.accentText })).setOrigin(0, .5));
    content.add(this.scene.add.text(-350, y + 174, t("interaction.exchange.remaining", { remaining: offer.remaining }), textStyle({ role: "body", size: 24 })).setOrigin(0, .5));
    ...
    content.add(new Button(this.scene, 230, y + 174, { width: 250, height: 76, label: this.pending ? t("interaction.exchange.busy") : t("interaction.exchange.do"), variant: "primary", onClick: () => void this.exchange(offer, quantity) }).setEnabled(!this.pending && offer.unlocked && offer.remaining >= quantity && offer.cost.owned >= offer.cost.amount * quantity));
  ...
  private async exchange(offer: InteractionExchangeOfferDto, quantity: number): Promise<void> { if (this.pending) return; this.pending = true; await this.refresh(); let result = ""; try { const receipt = await this.manager.exchange(offer.id, quantity, crypto.randomUUID()); result = t("interaction.exchange.granted", { amount: receipt.granted[0]?.amount ?? 0 }); } catch (error) { result = error instanceof Error ? error.message : t("interaction.exchange.failed"); } finally { this.pending = false; await this.refresh(result); } }
}
