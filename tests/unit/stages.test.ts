...
import { CHAPTERS, DAILY_RESTORATION, FIXED_STAGE_ENEMIES, STAGES, getStage, getStageEnemies } from "../../src/data/stages";
...
describe("stage enemy design", () => {
  ...
  it("모든 스테이지에 임시 적 세 명을 챕터별 순환 편성한다", () => {
    expect(STAGES.every((stage) => new Set(stage.enemies).size === FIXED_STAGE_ENEMIES.length)).toBe(true);
    expect(FIXED_STAGE_ENEMIES.map((id) => getRelic(id).name)).toEqual(["토비", "아모", "리파"]);
  });

  it("스테이지마다 적 레벨을 1 올리고 원본보다 강한 복사본을 만든다", () => {
    expect(STAGES.map((stage) => stage.enemyLevel)).toEqual(Array.from({ length: 30 }, (_, index) => index + 1));
    const finalEnemies = getStageEnemies(STAGES[29]);
  ...
  it("세 챕터의 본편 ID 30개를 유일한 평탄 인덱스로 제공하고 알 수 없는 ID를 거부한다", () => {
    ...
    expect(new Set(STAGES.map(({ id }) => id)).size).toBe(30);
