...
describe("relic catalog", () => {
  ...
  it("앞자리 0을 보존한 문자열 개체번호순으로 정렬한다", () => {
    ...
    // 신규 케리스 182번과 디안 221번도 앞자리 보존 문자열 순서의 끝에 안정적으로 정렬된다.
    // 델로피 013번은 배열 끝에 등록됐지만 앞자리 0을 보존한 채 001과 014 사이에 선다.
    // 테리사 078번도 배열에는 파루아 앞에 있지만 072와 083 사이에 선다.
    // 모르페 206번은 같은 시그널 아이인 슈테 205번 바로 뒤다.
    expect(sorted.map((relic) => relic.specimenNumber)).toEqual(["001", "013", "014", "015", "034", "038", "047", "059", "072", "078", "083", "093", "096", "105", "118", "126", "141", "163", "182", "205", "206", "221"]);
  });
  ...
  it("8번 Puppet 캐릭터 티아를 전용 원화·SD와 함께 공개한다", () => {
    ...
    for (const metadata of [TIA_PORTRAIT_METADATA, TIA_SD_METADATA]) {
      ...
      expect(metadata.content.bottom).toBeLessThanOrEqual(metadata.imageHeight);
      expect(metadata.content.right - metadata.content.left).toBeGreaterThan(metadata.imageWidth / 2);
    }
  });
  ...
  it("루카의 무리 사냥·치명적인 발톱·약점 관통 수치 계약을 공개한다", () => {
    ...
    expect(luka.ferocityTrait).toMatchObject({ name: "폭주", effectId: "packHunt", stealthDurationSeconds: 3, retriggerPackHunt: true, sharedTargetAttackSpeedPercent: 40 });
    expect(luka.passive).toMatchObject({ name: "무리 사냥", kind: "followHighestAttackAllyTarget" });
    expect(luka.basic).toMatchObject({ name: "치명적인 발톱", power: 100, periodicCritical: { every: 4 } });
    expect(luka.ultimate).toMatchObject({ name: "약점 관통", power: 200, cost: 90, blinkToLowestDefense: true, damageTransfer: { percent: 75, distanceOrigin: "primaryTarget" } });
  });
