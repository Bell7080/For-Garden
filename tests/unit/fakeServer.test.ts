...
describe("FakeServer", () => {
  ...
  it("v18 신규 재화 소급분은 서버 기준 시각과 보관 상한으로 한 번만 정산한다", async () => {
    ...
    // 24시간 미접속이어도 기본 보관 시간(8시간)만 계산하며, 같은 서버 시각의 재조회는 다시 지급하지 않는다.
    expect(first.excavation.unclaimed).toEqual(repeated.excavation.unclaimed);
    expect(first.excavation.unclaimed).toMatchObject({ rawStone: 10.56, gems: 4.48 });
  ...
  it("같은 수확 요청을 반복해도 한 번만 지급하고 지갑 상한을 넘기지 않는다", async () => {
    const state = makeSession(); state.wallet.gold = 999_999_998; state.idleExcavation.unclaimed = { gold: 5, cheesecake: 1, rawStone: 0, gems: 0 };
    ...
    expect(first.remaining).toEqual({ gold: 0, cheesecake: 0, rawStone: 0, gems: 0 });
  });
  ...
  it("두 기기의 연속 수확처럼 서로 다른 요청은 첫 호출만 기존 누적량을 받는다", async () => {
    const state = makeSession(); state.idleExcavation.unclaimed = { gold: 20, cheesecake: 3, rawStone: 0, gems: 0 };
    const server = new FakeServer(state, { latencyMs: 0, now: () => new Date("2026-08-20T00:00:00Z") });
    ...
    expect(firstDevice.granted).toEqual({ gold: 20, cheesecake: 3, rawStone: 0, gems: 0 });
    expect(secondDevice.granted).toEqual({ gold: 0, cheesecake: 0, rawStone: 0, gems: 0 });
  ...
  it("지갑이 이미 상한이면 지급량 0과 유실량을 구분해 반환한다", async () => {
    ...
    state.idleExcavation.unclaimed = { gold: 7, cheesecake: 2, rawStone: 0, gems: 0 };
    const result = await new FakeServer(state, { latencyMs: 0 }).harvestExcavation({ requestId: "full-wallet" });
    expect(result.granted).toEqual({ gold: 0, cheesecake: 0, rawStone: 0, gems: 0 });
    expect(result.discarded).toEqual({ gold: 7, cheesecake: 2, rawStone: 0, gems: 0 });
  });
  ...
  it("장시간 미접속 생산은 저장 시간 상한까지만 정산한 뒤 한 번에 지급한다", async () => {
    ...
    expect(result.granted.gold).toBe(1_071); expect(result.remaining.gold).toBe(0);
  ...
  it("현상수배는 세 라운드를 다 이긴 판에만 골드를 주고 다음 등급을 연다", async () => {
    ...
    expect(won).toMatchObject({ victory: true, goldEarned: 1_200, firstClear: true, clearedTierIds: ["bounty-1"] });
    expect(state.wallet.gold).toBe(1_200);
  ...
  it("현상수배 소탕은 이긴 등급만 전투 없이 횟수만큼 털고 같은 요청을 두 번 세지 않는다", async () => {
    const state = makeSession();
    state.wallet.stamina = 100;
    state.itemInventory = [{ itemId: "sweep-ticket", quantity: 4 }];
    const server = new FakeServer(state, { latencyMs: 0, now: () => new Date("2026-09-19T04:00:00Z") });
    await expect(server.sweepBounty({ tierId: "bounty-1", requestId: "early", count: 1 })).rejects.toMatchObject({ code: "DUNGEON_NOT_CLEARED" });
    ...
    expect(state.wallet.gold).toBe(4_800);
    expect(state.itemInventory).toEqual([{ itemId: "sweep-ticket", quantity: 1 }]);
  });
  ...
  it("서버 안에서 비용과 결과를 함께 확정한다", async () => {
    ...
    expect(response.wallet.fossil).toBe(999);
    ...
    expect(state.wallet.fossil).toBe(999);
    expect(state.gachaPityByGroup["standard-fossil"].pullsSinceSsr).toBe(0);
  });
...
describe("FakeServer 상품 카탈로그", () => {
  ...
  it("여러 개의 총가격·총 지급량·구매 제한을 검증한 뒤 한 번에 확정한다", async () => {
    // 무역 패키지는 젬으로 값을 받으므로 지갑의 젬만 채우고 화석은 그대로 둔다.
    const state = makeSession(1_000); state.wallet.gems = 1_000;
    const server = new FakeServer(state, { latencyMs: 0, now: () => new Date("2026-08-22T12:00:00Z") });
    const result = await server.purchaseProduct({ storefront: "trade", productId: "trade-cheesecake-supply", quantity: 2 });
    // 단가와 단위 지급량에 수량을 곱한 결과와 제한 기록이 같은 응답 시점에 반영된다.
    expect(result).toMatchObject({ quantity: 2, remaining: 1, wallet: { gems: 700, cheesecake: 1_200 }, granted: [{ kind: "currency", currency: "cheesecake", amount: 1_200 }] });
    // 주간 갱신이므로 기간 키는 그 주의 월요일(UTC)이다.
    expect(state.productPurchases["trade-cheesecake-supply"]).toEqual({ periodKey: "2026-08-17", count: 2 });
    const before = { wallet: { ...state.wallet }, purchases: { ...state.productPurchases } };
    await expect(server.purchaseProduct({ storefront: "trade", productId: "trade-cheesecake-supply", quantity: 2 })).rejects.toMatchObject({ code: "PURCHASE_LIMIT_REACHED" });
    expect(state.wallet).toEqual(before.wallet); expect(state.productPurchases).toEqual(before.purchases);
  });

  it("인게임 가격 차감, 지급, 주간 제한을 한 처리로 확정한다", async () => {
    const state = makeSession(1000); state.wallet.gems = 1_000;
    const server = new FakeServer(state, { latencyMs: 0, now: () => new Date("2026-08-22T12:00:00Z") });
    await expect(server.purchaseProduct({ storefront: "trade", productId: "trade-cheesecake-supply", quantity: 1 })).resolves.toMatchObject({ productId: "trade-cheesecake-supply", quantity: 1, remaining: 2, wallet: { gems: 850, cheesecake: 600 } });
    expect(state.productPurchases["trade-cheesecake-supply"]).toEqual({ periodKey: "2026-08-17", count: 1 });
    await server.purchaseProduct({ storefront: "trade", productId: "trade-cheesecake-supply", quantity: 1 });
    await server.purchaseProduct({ storefront: "trade", productId: "trade-cheesecake-supply", quantity: 1 });
    await expect(server.purchaseProduct({ storefront: "trade", productId: "trade-cheesecake-supply", quantity: 1 })).rejects.toMatchObject({ code: "PURCHASE_LIMIT_REACHED" });
  });

  it("재화 부족과 유료 상품은 어떤 지급도 만들지 않는다", async () => {
    const state = makeSession(0); const before = { ...state.wallet };
    const server = new FakeServer(state, { latencyMs: 0, now: () => new Date("2026-08-22T12:00:00Z") });
    await expect(server.purchaseProduct({ storefront: "trade", productId: "trade-cheesecake-supply", quantity: 1 })).rejects.toMatchObject({ code: "INSUFFICIENT_CURRENCY" });
  ...
  it("조회 storefront를 분리하고 다른 화면 상품 ID 구매를 거부한다", async () => {
    ...
    await expect(server.purchaseProduct({ storefront: "shop", productId: "trade-cheesecake-supply", quantity: 1 })).rejects.toMatchObject({ code: "PRODUCT_STOREFRONT_MISMATCH" });
  ...
  it("잘못된 수량은 차감 전에 거부한다", async () => {
    ...
    const server = new FakeServer(state, { latencyMs: 0, now: () => new Date("2026-08-22T12:00:00Z") });
    await expect(server.purchaseProduct({ storefront: "trade", productId: "trade-cheesecake-supply", quantity: 0 })).rejects.toMatchObject({ code: "INVALID_PURCHASE_QUANTITY" });
    await expect(server.purchaseProduct({ storefront: "trade", productId: "trade-cheesecake-supply", quantity: 1.5 })).rejects.toMatchObject({ code: "INVALID_PURCHASE_QUANTITY" });
...
describe("FakeServer 광고 보상 경계", () => {
  ...
  it("1.5배는 현재 확정 수확에 한 번만 적용하고 다음 수확에는 남지 않는다", async () => {
    const state = makeSession(); state.idleExcavation.unclaimed = { gold: 10, cheesecake: 2, rawStone: 0, gems: 0 };
    const server = new FakeServer(state, { latencyMs: 0, now: () => new Date("2026-08-22T12:00:00Z") });
    ...
    expect(boosted.granted).toEqual({ gold: 15, cheesecake: 3, rawStone: 0, gems: 0 });
    state.idleExcavation.unclaimed = { gold: 10, cheesecake: 2, rawStone: 0, gems: 0 };
    const normal = await server.harvestExcavation({ requestId: "normal-harvest" });
    expect(normal.granted).toEqual({ gold: 10, cheesecake: 2, rawStone: 0, gems: 0 });
  });
  ...
  it("패스 즉시 수령도 발굴 효과와 슬롯별 UTC 제한을 그대로 적용한다", async () => {
    ...
    expect(result.reward).toMatchObject({ kind: "excavation_effect", effect: { kind: "storage_extension", maxStorageSeconds: 57_600 } });
    expect(state.idleExcavation.storageExtensionExpiresAt).toBe("2026-08-23T04:00:00.000Z");
  });
});
