...
export const KEYWORDS: readonly KeywordDef[] = [
  ...
  {
    // 태그는 이 표식이 무엇이고 언제 사라지는지까지만 말한다. **남기는 것은 패시브의 몫이고**,
    // 지워질 때 무엇이 함께 터지는지는 그 타격을 내는 스킬이 말한다 — 일반 공격과 궁극기가
    // 서로 다르기 때문이다.
    id: "shimmer", term: "반짝!", kind: "debuff",
    description: "물살이 튀어 묻은 표식이다. 반짝이 묻은 적이 다시 맞으면 그 반짝은 사라진다.",
  },
  ...
  {
    /*
     * 집중은 공격력만이 아니라 사거리를 함께 올린다 — 쏠수록 더 멀리 닿는 것이 이 규칙의 전부다.
     *
     * **수치의 출처는 여기가 아니다.** 겹당 공격력은 `farthestFocus` 패시브의 `value`, 겹당
  ...
  },
  {
    ...
    description: "적 주위에 고이는 얕은 물이다. 잠긴 적은 이동 속도가 35% 느려지고, 물 밖으로 나가면 곧바로 풀린다. 아무도 붙지 않은 적 주위가 먼저 젖으며, 이미 고인 판과 가까우면 새로 생기지 않고 그 판이 갱신된다.",
  },
  // 강하는 여울과 갈라 둔다. 하나로 묶었을 때는 태그 한 장이 "물이 무엇인가"와 "언제 어떻게
  // 뛰는가"를 함께 말해, 다른 규칙어의 두세 배 길이가 되고 정작 둘 다 흐려졌다.
  {
    id: "dive", term: "강하", kind: "rule",
    description: "적이 가장 많이 잠긴 [[shallows|여울]]로 순간이동해 그 자리를 터뜨린다. 그때 잠겨 있던 적은 40% 더 받으며, 터진 판은 마른다. 아무도 잠기지 않았으면 뛰지 않는다.",
  },
  {
    id: "combo", term: "연격", kind: "rule",
    description: "한 번의 기본 공격 행동이 여러 번 적중한다. 각 타격이 적중 효과를 따로 발동한다.",
  },
  {
  ...
  },
  {
    ...
    term: "목덜미",
    kind: "rule",
    description: "약해진 적을 끝내는 마무리 한 방이다. 확정 치명타가 되고 피해가 크게 늘어난다.",
  },
  {
