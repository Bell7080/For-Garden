...
describe("폰토스 실전 스킬과 심해 압력", () => {
  ...
  it("는 체력이 닳을수록 경감을 곡선으로 올리고 절반에서 막히지 않는다", () => {
    ...
    expect(receivedDamage(pontos, 100)).toBe(48); // 직선이던 때는 26이었다.
    pontos.hp = pontos.maxHp * 0.5;
    expect(receivedDamage(pontos, 100)).toBe(39); // 절반에서 상한에 닿지 않는다.
    pontos.hp = pontos.maxHp * 0.25;
    expect(receivedDamage(pontos, 100)).toBe(24);
    // 절반 아래에서도 계속 자란다 — 멈추는 자리와 끝나는 자리가 같다.
    pontos.hp = pontos.maxHp * 0.1;
    expect(receivedDamage(pontos, 100)).toBe(11);
  });
