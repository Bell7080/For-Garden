...
export class SaveManager {
  ...
  private toSaveData(state: Session): SaveData {
    const data: SaveData = {
      ...
      equippedRelicSkinIds: { ...state.equippedRelicSkinIds },
      ...
      wallet: { ...state.wallet },
      // 로컬에서는 서버가 마지막으로 확정한 기준 시각만 그대로 보존한다.
  ...
  migrate(input: unknown): SaveData {
    ...
    const savedGroups = legacy.gachaPityByGroup && typeof legacy.gachaPityByGroup === "object"
      ? legacy.gachaPityByGroup as Record<string, { pullsSinceSsr?: unknown; pickupGuaranteed?: unknown; totalPulls?: unknown }> : {};
    ...
    const normalizedPity = Object.fromEntries(groupIds.map((groupId) => {
      ...
      // 횟수 제한 배너의 누적 횟수(v41, 첫 복원 연구)는 있으면 그대로 보존한다.
      const totalPulls = Number.isInteger(saved?.totalPulls) ? { totalPulls: saved!.totalPulls as number } : {};
      return [groupId, { pullsSinceSsr: saved?.pullsSinceSsr ?? legacyCount, pickupGuaranteed: saved?.pickupGuaranteed ?? false, ...totalPulls }];
    }));
    ...
    runeInventory = runeInventory.map((rune, index) => {
      ...
      return Number(legacy.saveVersion) <= 27 ? { ...normalized, sequence: Number.isSafeInteger(normalized.sequence) ? normalized.sequence : index } : normalized;
    });
  ...
  validate(data: SaveData): void {
    ...
    const skinsById = new Map(RELIC_SKINS.map((skin) => [skin.id, skin]));
  ...
  private toSession(data: SaveData): Session {
    return {
      ...
      ownedRelicSkinIds: new Set(data.ownedRelicSkinIds),
      equippedRelicSkinIds: { ...data.equippedRelicSkinIds },
      ...
      wallet: { ...data.wallet }, staminaUpdatedAt: data.staminaUpdatedAt, relicProgress: Object.fromEntries(Object.entries(data.relicProgress).map(([id, value]) => [id, cloneProgress(value)])),
