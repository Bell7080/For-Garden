...
export const SHOP_ENTRANCE = {
   ...
   *
   * **`merchantWait`는 점원을 기다려 주는 한계다.** 첫 마디는 점원이 도착한 순간에 함께
   * 서지만, 묶음이 느리거나 아예 오지 못하면 그 문이 영영 열리지 않아 **첫 마디가 통째로
   * 사라진다** — 눌러야만 말이 나오는 화면이 된다. 전시대가 다 올라온 뒤로 이만큼 지나면
   * 점원 없이도 말이 선다: 말 없는 점원보다 아직 그려지지 않은 점원이 낫다.
   */
  dialogue: { slide: -240, delay: 430, merchantWait: 2600 },
} as const;
