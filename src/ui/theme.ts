...
export const COLOR = {
  ...
  hpEnemy: 0xb03a3a,
  /**
   * 보호막은 체력 **위에 이어 붙는 푸른 칸**이다. 체력을 덮어쓰지 않고 그 오른쪽으로 자라야
   * "지금 몸이 얼마나 남았나"와 "그 앞에 무엇이 한 겹 있나"가 따로 읽힌다.
   */
  shieldFill: 0x7fb4ec,
  /** 같은 막을 알리는 피해 수치 글자색. 게이지와 반드시 같은 색이어야 둘이 한 사건으로 읽힌다. */
  shieldText: "#7fb4ec",
  energy: 0x7ea8d8,
  ...
  sortieText: "#f08b62",
  /**
   * 교환하는 자리의 강조색 — 교류와, 판 밖 곁들임 줄에 서는 가게 입구가 함께 쓴다.
   *
   * 출격의 주황과 마주 보는 자리라 성격이 다른 색을 쓰고, 두 화면(출격판·레이드)의 상점
   * 입구가 한 값을 읽어 같은 문으로 읽힌다.
   */
  exchange: 0x6fa8d6,
  exchangeText: "#9fd0f0",
