...
export const KEYWORDS: readonly KeywordDef[] = [
  ...
  {
    // 쓰는 개체가 티아 하나뿐이라 태그가 규칙을 통째로 갖는다(덧칠·손질과 같은 처리). 둘째
    // 개체가 생기면 출혈처럼 수치를 본문으로 옮긴다. 터질 때 무엇이 함께 오는지는 그 타격을
    // 내는 스킬이 말한다 — 일반 공격과 궁극기가 지우는 값이 서로 다르기 때문이다.
    id: "shimmer", term: "반짝!", kind: "debuff",
    description: "물살이 튀어 묻은 표식이다. 반짝이 없는 적을 때리면 반짝을 남기며 주문력의 100%만큼 마법 피해를 추가로 입히고, 반짝이 묻은 적을 때리면 그 반짝이 사라진다.",
  },
