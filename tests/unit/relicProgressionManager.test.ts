...
describe("RelicProgressionManager", () => {
  it("미보유 렐릭의 성장 정보를 읽어도 세션을 변경하지 않는다", () => {
    ...
    // 기본 해금에 들어 있지 않은 티아를 사용해 미보유 조회 계약을 계속 검증한다.
    expect(state.owned.has("tia")).toBe(false);
    expect(manager.getProgress("tia")).toMatchObject({ level: 1, breakthrough: 0 });
    expect(state.relicProgress).not.toHaveProperty("tia");
  });
  ...
  it("미보유 렐릭의 성장 상태 변경을 거부한다", () => {
    ...
    expect(() => manager.setLevel("tia", 2)).toThrow("보유하지 않은 렐릭");
  });
});
