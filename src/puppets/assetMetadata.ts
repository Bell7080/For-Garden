...
export const NODONIA_PORTRAIT_METADATA: Omit<PuppetAsset, "url"> = {
   ...
   */
  /**
   * 카드 배율은 `cardZoom × portraitZoom`의 곱으로 정해진다. 정보창에서 되돌린 몫(1.55)이
   * 카드에도 함께 들어오므로, 카드에서 실제로 쓰는 배율(1.5)이 되도록 여기서 나눠 둔다.
   */
  cardZoom: 1.034,
  /**
   * 정보창 전신 배율.
   *
   * 카드와 같은 이유로 얼굴이 혼자 작다 — 펼친 날개와 베일이 실루엣을 키워 놓아, 그림 전체를
   * 공용 높이에 맞추면 정작 얼굴은 다른 개체의 63%밖에 되지 않았다(눈 간격 61.5px, 중앙값
   * 106px). 1.45를 곱해 렉시아(87)·메론(93) 사이(89px)에 세운다 — 한 번 1.55(95px)까지
   * 올렸다가 화면에서 커 보여 한 뼘 되돌렸다.
   */
  portraitZoom: 1.45,
  /**
