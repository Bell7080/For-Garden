...
- **적은 이 정보창을 쓰지 않는다.** 지도·편성·전투 어디서든 적을 누르면 `src/ui/EnemyStatusWindow.ts`
  한 장이 열린다. 급여·돌파·유대·룬이 없는 화면을 따로 두는 이유는, 같은 판을 문맥만 바꿔
  재사용하면 "이 적에게 치즈케이크를 먹일 수 있나" 같은 물음이 화면에 남기 때문이다.
  적 수치는 `getStageEnemies`가 만든 레벨 보정 정의를 그대로 읽고 창이 다시 보정하지 않는다.
...
- 궁극기 연출은 **짧다.** 화면을 덮는 컷인도 포효도 두지 않고, 때리는 개체가 한 뼘 커졌다가
  세게 치고 곧바로 전투가 이어진다. 배율·시간·흔들림은 `src/data/ultimatePresentations.ts`
  한 표에만 있고 씬은 그 값을 읽기만 한다 — 전투 중 여러 번 반복되는 연출이라 길이가 곧
  기다림이다.
