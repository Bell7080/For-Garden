...
export const RELICS: RelicDef[] = [
  ...
  },
  {
    ...
    basic: {
       ...
       * 들어간다 — 「악어턱」이라는 이름이 실제로 붙잡는 순간은 여기뿐이다. 대신 그 값은
       * **자리에** 걸려 있어, 달려가는 동안과 물이 마른 뒤에는 예전과 같은 40%로 돌아간다.
       *
       * **연격에 달려 있던 회복(적중마다 잃은 체력 5%)은 걷어 냈다.** 암살자의 "낮은
       * 생존력"이 그 한 줄로 통째로 뒤집혀, 붙어 싸울수록 차오르는 몸이 되었기 때문이다.
       * 그 몫은 회복이 아니라 **잠긴 적에게 주는 피해 20%**로 돌아왔다 — 같은 값을 순간
       * 화력으로 돌려주므로 암살자의 자리는 지키면서, 상시 배율이 아니라 **지금 어디에 서
       * 있느냐**가 세기를 정한다. 렉시아가 물어뜯어 출혈로 깎아 가는 개체라면 이쪽은
       * 물가로 끌어들여 빠져나가지 못하게 하고 그 자리에서 터뜨리는 개체다.
       */
      shallows: {
        radius: 200,
        seconds: 3,
        ...
        /*
         * **물가에서는 네 번 문다.** 물 밖에서는 40% 확률로 두 대뿐이라, 이 둘의 차이가 곧
         * "사냥감을 가둔 값"이다. 여울이 평타를 한 번 더 넣어 주는 장판에서 **이 개체가
         * 화력을 내는 자리**로 옮겨 오는 수치이고, 「전투의 환희」가 쌓은 공격 속도가 그
         * 위에서 곱해져 물가에 오래 갇힌 적일수록 빠르게 녹는다.
         */
        submergedHitCount: 4,
        /*
         * 메워 준 두 대는 얕게 문다. 타수만 늘리면 화력이 그대로 두 배가 되어 **11레벨 스피나
         * 한 명이 혼자 2장을 끝냈다**(`하이퍼 캐리 혼자서는 2장을 끝내지 못한다` — 100% 클리어).
         * 편성 칸이 셋인 이유를 지우는 값이라 여기서 끊는다.
         *
         * **이 한 수가 1대1 세기와 혼자 미는 힘을 동시에 움직인다.** 실측한 곡선은
         * 30% → 스피나 74%(솔로 계약 통과) · 40~60% → 76~80%(솔로 2-10이 열림) ·
         * 70% → 85%(솔로 2-10 전승)다. 범람의 넓이·시간은 1대1에서 표적이 늘 판 안이라
         * 거의 움직이지 않으므로 조절 손잡이가 되지 못한다.
         *
         * 30%를 고른 이유는 **편성 칸 셋이 이 개체 하나보다 앞서기** 때문이다. 화면에는 네 번
         * 튀고 총합은 2.0 → 2.6대만 오른다 — 여울은 "피해가 두 배가 되는 장판"이 아니라
         * **더 자주, 얕게 무는 자리**다.
         */
        submergedExtraHitPowerPercent: 30,
        submergedDamagePercent: 20,
      },
    ...
    ultimate: {
      ...
      /*
       * **범람.** 이 궁극기의 값은 한 명을 세게 치는 것이 아니라 **사냥터를 여는 것**이다.
       *
       * 평타가 발밑에 찔끔 고이는 반경 200·3초짜리 판을 반경 500·6초로 덮으므로, 기절한
       * 표적만이 아니라 그 둘레의 적이 전부 잠긴다 — 뒤이어 나가는 평타가 넷씩 들어가고
       * 잠긴 쪽은 이동이 35% 느려 물 밖으로 걸어 나가지도 못한다. 연격에서 걷어 낸 회복의
       * 자리를 메우는 것이 이 창이고, 그래서 이 개체의 화력은 **얼마나 오래 가둬 두었나**로
       * 읽힌다.
       *
       * 반경을 절대값이 아니라 배수로 적는 이유는 평타 여울을 고치면 범람도 함께 움직여야
       * 두 판이 같은 물로 읽히기 때문이다.
       */
      floodShallows: { radiusMultiplier: 2.5, seconds: 6 },
  ...
  },
  {
    ...
    passive: {
      ...
      /*
       * 로스터 최저 이동 속도로 **가장 먼 적**을 노리는 개체라, 여는 5초가 곧 사거리다.
       * 집중은 적중해야 쌓이는데 시작하자마자 표적이 되면 한 겹도 쌓기 전에 붙잡힌다 —
       * 숨어서 첫 몇 발을 먼저 꽂아야 "쏠수록 멀리 닿는다"가 실제로 열린다.
       *
       * 델로피(5초)보다 한 뼘 짧다. 그쪽은 여는 은신이 곧 기습이라 나오는 순간 터뜨리지만,
       * 이쪽은 **자리를 잡는** 시간이라 길수록 집중이 무방비로 쌓인다 — 5초로 두었을 때는
       * 1장 전체가 바닥 로스터에게 전승으로 열렸다(1-7·1-9가 0.000 → 1.000).
       */
      openingStealthSeconds: 4,
      // 전용 분기가 있으므로 화면에는 이 문장이 아니라 `passiveDescription`이 지은 것이 뜬다.
      // 여기 사본은 데이터만 읽는 사람을 위한 것이라, 분기를 고칠 때 함께 고친다.
      desc: "전투 시작 시 4초 동안 은신하며, 사거리 안에서 가장 먼 적을 노리고 적중할 때마다 집중을 얻어 공격력과 사거리가 오른다.",
    },
    basic: {
      id: "parua-basic",
      name: "신중한 일격...! 이에요...",
  ...
  SHIRO_DEF,
  {
    ...
    passive: {
      ...
      desc: "매초 주문력이 2%씩 복리로 오른다. 체력이 낮아질수록 받는 모든 피해가 50%에서 99%까지 줄고, 10 이하의 피해는 무효가 된다.",
    },
    basic: {
      id: "pontos-basic",
      name: "심해 충격파",
