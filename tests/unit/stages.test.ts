...
describe("stage enemy design", () => {
  ...
  it("챕터 1의 레벨·돌파 초안을 적별 StageEnemyDef에 정확히 기록한다", () => {
     ...
     * 하나가 대신하기 때문이다.
     *
     * **칸은 둘뿐이다 — 자란 레벨과 한계 돌파.** 예전에는 셋째 칸에 야성 단계가 있었고 그
     * 수에 곱해지는 배율(잡졸 ×3 · 정예 ×5)이 화면에 없어 `LV.7 +20`이 실제로는 107레벨이었다.
     * 지금 여기 적힌 수가 곧 그 개체가 싸우는 레벨이고, 정예가 혼자 서는 몫은 유형 표
     * (`ENCOUNTER_ROLE.elite`)의 체력·공격 배수가 낸다.
     */
    expect(ladder).toEqual([
      // 1-1은 오프닝에서 곧장 들어오는 첫 전투라 레벨 1 기본 편성이 넘되 1-2(8)와 턱이 크지 않도록 5다.
      { 아모: [5, 0], 토비: [5, 0], 리파: [5, 0] },
      { 아모: [8, 0], 토비: [8, 0], 리파: [8, 0] },
      // 1-3부터는 실제 전장 크기로 잰 검수에 맞춰 한 뼘씩 올렸다(v0.172.6) — 키우지 않은
      // 파티가 1장 중반부터 조이고 후반에 막히는 선이다.
      { 아모: [11, 0], 토비: [11, 0], 리파: [11, 0] },
      { 아모: [13, 0], 토비: [13, 0], 리파: [13, 0] },
      { 토비: [15, 0] },
      { 아모: [17, 0], 토비: [17, 0], 리파: [17, 0] },
      { 아모: [18, 0], 토비: [18, 0], 리파: [18, 0] },
      { 아모: [19, 0], 토비: [19, 0], 리파: [19, 0] },
      { 아모: [20, 0], 토비: [20, 0], 리파: [20, 0] },
      { 코마: [20, 0] },
    ]);
    /*
    ...
    /*
     * **정예는 직전 관문보다 가볍게 서지 않는다** — 다만 그 무게는 레벨이 아니라 **실제 체력**
     * 으로 잰다. 혼자 서는 몫(`ENCOUNTER_ROLE.elite`의 3.6배)을 유형이 내므로 정예의 레벨이
     * 직전 잡졸보다 낮은 자리가 있고, 레벨만 비교하면 그 관문이 "약해졌다"로 읽힌다.
     * 마지막 관문의 코마가 레벨 1로 남아 실제로 쉬웠던 적이 있다(v0.77.2에서 고쳤다).
     */
    const toughest = (index: number) => Math.max(...getStageEnemies(battles[index]).map(({ stats }) => stats.hp));
    expect(toughest(4)).toBeGreaterThan(toughest(3));
    expect(toughest(9)).toBeGreaterThan(toughest(8));
    /*
  ...
  it("1-1부터 1-10까지 재등장한 캐릭터의 레벨이나 돌파가 메타데이터 없이 역행하지 않는다", () => {
    ...
    for (const stage of battles.slice(0, 10)) {
      for (const enemy of stage.enemies) {
        ...
        /*
         * **정예 자리는 이 줄 밖이다.** 정예는 권장 레벨에 유형 차(+3)가 붙어 서므로, 같은
         * 개체가 다음 관문에 잡졸로 돌아오면 레벨이 내려간 것처럼 보인다 — 그 자리가 무거운
         * 몫은 레벨이 아니라 `ENCOUNTER_ROLE.elite`의 체력·공격 배수가 낸다.
         */
  ...
  it("스테이지 사이에서 허용하는 차이는 레벨·돌파뿐이다", () => {
    // 태생 능력치나 스킬 보정을 스테이지에 두지 않는다는 계약이다. 유형이 주는 몫은 관문이
    // 아니라 `ENCOUNTER_ROLE` 한 표가 갖는다.
  ...
  it("정예 관문의 적 레벨은 권장 레벨에 유형 차 하나만 얹는다", () => {
    /*
     * 예전에는 이 자리에 야성 단계가 있었고 잡졸 ×3 · 정예 ×5로 얹혀, 화면의 `LV.7 +20`이
     * 실제로는 107레벨이었다. 지금은 화면에 선 수가 곧 싸우는 수이고, 혼자 서는 몫은
     * `ENCOUNTER_ROLE.elite`의 체력·공격 배수가 갖는다.
     */
  ...
  it("적 레벨은 한계 돌파로 닿을 수 있는 자리에만 선다", () => {
    /*
     * **레벨 사다리는 돌파 사다리 위에 놓인다.** 상한이 20인 사람에게 권장 28짜리 관문을
     * 세우면 그 관문은 "더 키우면 된다"가 아니라 막힌 문이다. 1장은 돌파 없이 끝나고,
     * 뒤 장은 그 시점에 뚫려 있을 만한 단계 안에 든다.
     */
    for (const stage of battles) {
      for (const enemy of stage.enemies) {
        const breakthrough = requiredBreakthroughForLevel(enemy.level);
        expect(enemy.level, stage.id).toBeLessThanOrEqual(relicLevelCap(breakthrough));
      }
    }
    const chapterOne = battles.filter(({ id }) => id.startsWith("1-"));
    for (const stage of chapterOne) {
      for (const enemy of stage.enemies) expect(requiredBreakthroughForLevel(enemy.level), stage.id).toBe(0);
    }
  });

  it("적 레벨은 관문 순서를 따라 내려가지 않는다", () => {
    /*
     * 레벨이 뒤로 가면 새 구역이 직전 구역보다 약해 곡선이 끊긴다.
     *
     * **정예 관문은 이 사다리 밖이다.** 혼자 서는 몫을 유형 배수가 내므로 같은 레벨이라도
     * 훨씬 무겁다 — 그 수를 잡졸의 줄에 끼워 비교하면 다음 관문이 언제나 "약해졌다"로 읽힌다.
     */
    let previous = 0;
    for (const stage of battles) {
      for (const enemy of stage.enemies) expect(Number.isInteger(enemy.level), stage.id).toBe(true);
      if (stage.kind === "battle" && stage.elite === true) continue;
      const level = Math.max(...stage.enemies.map((enemy) => enemy.level));
      expect(level, stage.id).toBeGreaterThanOrEqual(previous);
      previous = level;
    }
  });
  ...
  it("모든 적의 레벨과 돌파 단계가 성장 범위의 정수다", () => {
    for (const stage of battles) for (const enemy of stage.enemies) {
      ...
      // 적은 제 성장 곡선을 쓰므로 플레이어의 돌파 상한에 묶이지 않는다. 게임 전체의
      // 천장(돌파를 다 뚫은 상한)만 넘지 않으면 된다 — 그 위는 아무도 따라잡을 수 없다.
      expect(enemy.level).toBeLessThanOrEqual(relicLevelCap(BREAKTHROUGH_CAP));
    }
  });
   ...
   *
   * 적의 레벨은 제 곡선 위의 수이지만, 그 관문의 **권장 레벨**은 플레이어가 그 시점에 실제로
   * 닿을 수 있어야 한다. 돌파 0의 상한은 20이고 한 단계마다 30·40·50·60이 열리므로
   * (`BREAKTHROUGH_STEPS`), 1장은 돌파 없이 끝나고 2장이 1~2단계, 3장이 2~3단계를 전제한다.
   */
  it("관문의 권장 레벨은 그 장에서 뚫려 있을 만한 돌파 안에 든다", () => {
