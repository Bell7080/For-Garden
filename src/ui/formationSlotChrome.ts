...
export function addFormationSlotPlate(
  ...
  if (options.recommendedRoles && options.recommendedRoles.length > 0) {
    addRecommendedRoleHint(scene, parent, box, options.recommendedRoles);
    return;
  }
...
/** 빈 칸 가운데의 추천 직군 — 「추천 직군」 한 줄 아래 직군 아이콘이 나란히 선다. */
function addRecommendedRoleHint(
  scene: Phaser.Scene,
  parent: Phaser.GameObjects.Container,
  box: FormationSlotBox,
  roles: readonly Role[],
  ...
  const hint = scene.add.container(box.x, box.y).setName("formation-role-hint").setAlpha(spec.alpha);
  const label = scene.add.text(0, 0, t("formation.roleHint"), textStyle({ role: "emphasis", size: spec.fontSize, color: COLOR.inkDim, align: "center" })).setOrigin(0.5);
  squeezeTextToWidth(label, box.width - 16, 0.7);
  const blockHeight = spec.fontSize + spec.lineGap + spec.iconSize;
  label.setY(-blockHeight / 2 + spec.fontSize / 2);
  hint.add(label);
  const iconsWidth = roles.length * spec.iconSize + (roles.length - 1) * spec.gap;
  const iconY = blockHeight / 2 - spec.iconSize / 2;
  roles.forEach((role, index) => {
    hint.add(new AffinityBadge(scene, -iconsWidth / 2 + spec.iconSize / 2 + index * (spec.iconSize + spec.gap), iconY, ROLE_ICON[role], spec.iconSize, 0.4));
  });
  parent.add(hint);
}
