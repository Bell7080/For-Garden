...
describe("stage enemy design", () => {
  ...
  it("스테이지가 들고 다니는 야성 값은 곱하기 전의 단계다", () => {
    ...
    expect(elite.enemies[0].ferocityLevel).toBe(12);
    expect(effectiveEnemyLevel(elite.enemies[0], true)).toBe(elite.enemies[0].level + 60);
    // 정예 배율은 그 관문에만 든다. 같은 값이라도 잡졸로 세면 세 배다.
    expect(effectiveEnemyLevel(elite.enemies[0])).toBe(elite.enemies[0].level + 36);
