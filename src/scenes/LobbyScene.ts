...
export class LobbyScene extends Phaser.Scene {
  ...
  private openSortieMenu(instant = false): void {
    ...
    this.popupLayer.open({ width: panel.width, height: panel.height, y: panelY, title: t("lobby.sortie.title"), titleSize: 34, dim: true, dimAlpha: SORTIE_MENU.dimAlpha, closeOnBackdrop: false, hideCloseButton: true, instant, onClose: () => this.clearSortieChrome() }, (body, close) => {
  ...
  private addSortieShopPanel(body: Phaser.GameObjects.Container, shops: readonly { label: string; onClick: () => void }[]): void {
    ...
    shops.forEach((entry, index) => {
      ...
      hit.on("pointerdown", () => row.setScale(1.04));
      hit.on("pointerout", () => row.setScale(1));
      hit.on("pointerup", () => { row.setScale(1); entry.onClick(); });
      row.add(hit);
      body.add(row);
    });
  }
