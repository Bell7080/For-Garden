...
describe("현상수배 등급 표", () => {
  ...
  it("은 등급이 오를 때 어느 라운드도 가벼워지지 않는다", () => {
    // 실효 레벨이 한 번이라도 내려가면 더 높은 등급이 더 쉬운 자리가 생긴다.
    for (let round = 0; round < BOUNTY.roundCount; round += 1) {
      const levels = BOUNTY_TIERS.map((tier) => effectiveEnemyLevel(tier.rounds[round], true));
  ...
  it("은 등급 표가 적는 수를 곱하지 않은 **단계** 그대로 둔다", () => {
    // 화면의 붉은 `+n`이 읽는 값이라, 배율을 먹인 값을 데이터에 적으면 레벨과 나란히 읽힌다.
    for (const tier of BOUNTY_TIERS) for (const round of tier.rounds) expect(round.ferocityLevel).toBeLessThanOrEqual(10);
  });
});
