...
export const RELICS: RelicDef[] = [
  ...
  SHIRO_DEF,
  {
    ...
    passive: {
       ...
       * 맞았다 — 원정에서는 체력이 무한이라 그 값이 **판 내내 쓰이는 유일한 값**이라,
       * 사실상 "절반만 아픈 벽"이었다. 죽지 않는 벽이라면 처음부터 단단해야 한다.
       */
      baseDamageReductionPercent: 70,
      ...
      /*
       * **1보다 작아 초반에 붙고 뒤에서 완만해진다.** 체력 75%에서 80%, 50%에서 87%,
       * 25%에서 93%, 10%에서 97%로 오른다 — 남은 체력을 깎을수록 계속 오르되 오르는 폭이
       * 점점 줄어, 끝에서 상한에 **부딪히지 않고 닿는다.**
       *
       * 1보다 큰 값(2.2)을 썼던 때는 반대 모양이라 바닥에서 한참을 낮게 누워 있었다.
       * 바닥을 70으로 올린 지금은 그 모양이 오히려 "처음에만 물렁한 벽"이 된다.
       */
      damageReductionCurve: 0.75,
      /*
  ...
  },
  {
    ...
    stats: {
       ...
       *
       * **레이드 보스로 설 때의 그 훨씬 느린 걸음은 여기 적지 않는다** — 거대하고 느린 것은
       * 이 개체의 성질이 아니라 **레이드라는 자리의 성질**이라 무리 유형 표
       * (`ENEMY_PRESENCE.raid`)가 얹는다. 개체에 적으면 같은 몸이 도감과 관문에 설 때까지
       * 함께 느려진다.
       */
      attackSpeed: 48,
      moveSpeed: 52,
