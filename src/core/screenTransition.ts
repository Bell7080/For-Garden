...
export const TRANSITION = {
  ...
  /**
   * 한 화면 안에서 라벨 탭으로 목록을 갈아 끼울 때 — 상점·고고학·프리미엄·가방·레이드·환경설정.
   *
   * **탭이 선 쪽에서 들어온다.** 오른쪽 탭을 누르면 새 목록이 오른쪽에서 밀려 들어와, 줄의 어느
   * 칸으로 옮겼는지를 움직임이 말한다(핵심 화면 다섯이 좌우로 지나가는 것과 같은 문법). 목록만
   * 움직이고 탭 줄·상단 줄은 제자리다 — 그 둘까지 흔들리면 무엇이 바뀐 것인지 흐려진다. 핵심
   * 화면 전환보다 짧고 가깝다: 화면이 아니라 판 한 장이 갈리는 것이라서다.
   */
  tabPage: { duration: 170, distance: 56, alpha: 0.35, scale: 1 },
