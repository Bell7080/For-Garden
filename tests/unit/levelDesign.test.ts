...
const AUDIT: readonly [string, number, () => readonly RelicDef[], EncounterRole, number, number][] = [
  ...
  ["대작전 1단계 1파", 5, () => cakeOperationWaves(CAKE_OPERATION_TIERS[0])[0], "swarm", 14.3, 1.0],
  ["대작전 8단계 1파", 45, () => cakeOperationWaves(CAKE_OPERATION_TIERS[7])[0], "swarm", 27.4, 0.94],
...
describe("콘텐츠별 전투 시간 실측", () => {
  ...
  it("은 아직 목표 띠에 다 들어오지 않았고, 어디가 남았는지 이 줄이 기록한다", () => {
     ...
     * 대작전 첫 단계만 아직 셋씩 나오는 무리라 한 파가 아무것도 깎지 못한다 — 파 구성을 넷
     * 이상으로 올리는 것이 답이지 배수를 키우는 것이 아니다.
     ...
     * 나오는 무리라 한 파가 아무것도 깎지 못한다. 원정 정예는 아직 재측정이 남았다.
     */
    ...
    expect(offTarget.map(([label]) => label)).toEqual([
      "스토리 1-5 정예", "현상수배 1단계", "현상수배 5단계", "대작전 1단계 1파", "원정 정예 10층",
    ]);
  });
});
