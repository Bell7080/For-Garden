 ...
 *
 * **여기 서는 수는 그 개체가 실제로 싸우는 레벨 하나뿐이다.** 예전에는 자란 레벨 옆에 야성
 * 단계가 작고 붉은 `+n`으로 함께 섰는데, 그 수에 곱해지는 배율(잡졸 ×3 · 정예 ×5)이 화면에
 * 없어 `LV.7 +20`이 실제로는 107레벨이었다 — 읽을 수 없는 수는 없는 수와 같다. 지금은 세기의
 * 몫을 유형 표(`ENCOUNTER_ROLE`)가 갖고 레벨은 곧이곧대로 선다.
 *
