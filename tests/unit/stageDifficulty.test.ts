 ...
 * - **정예 둘만 막는다.** 관문의 무게는 야성 단계 하나로 내며(`ferocityBonusLevels`),
...
const BASELINES = {
   ...
   */
  /*
   * **정예에 무리 유형의 걸음이 붙으면서 다시 녹화했다**(v0.162.0). 정예는 이제 공속·이속이
   * 5% 빠르다(`ENEMY_PRESENCE.elite`) — 세기가 아니라 **인상**이라 같은 수치를 가진 잡졸과
   * 나란히 두었을 때 어느 쪽이 정예인지 서 있는 모습으로 갈린다. 그만큼 야성 단계를 덜어
   * (1-5 20 → 19, 1-10 14 → 12) 두 관문이 막는 자리는 그대로 두었다.
   */
  "1-5": { win: 1, hp: [0.23, 0.35] },
  "1-10": { win: 0.75, hp: [0.13, 0.25] },
...
describe("Phaser 없는 챕터 난이도 검수", () => {
  ...
  it("정예 두 관문만 조합을 가린다", () => {
    ...
    expect(winRateAt("1-10", earthDps)).toBe(0.75);
