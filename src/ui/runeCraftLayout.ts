...
export const RUNE_CRAFT_PANEL = {
   ...
   * 액자와 이름줄.
   *
   * 액자는 **깎인 왼쪽 위 모서리 안에** 앉아야 한다(`fitsInsidePopupBevel`). 판이 940×968 이상이라
   * 깎임이 131.6px이고, 예전 자리(-378, 126)는 액자 윗변이 그 대각선을 45.6px 넘어 판 밖에
   * 떠 있었다. 이름줄의 `textX`도 액자 오른쪽 변에서 한 뼘 떨어져야 한다 — 예전 -324는 액자
   * 오른쪽(-312)보다 왼쪽이라 등급·이름 글자가 조각 위에 겹쳐 찍혔다.
   */
  frame: { x: -338, y: 156, size: 132 },
  textX: -246,
  rarityY: 88,
  nameY: 116,
  nameWrap: 300,
...
export const RUNE_NOTE_PANEL = {
  ...
  /** 액자 오른쪽 변(-136)에서 한 뼘 떨어진다 — 붙이면 등급·이름 글자가 조각에 닿는다. */
  textX: -112,
  rarityY: 148,
  nameY: 174,
  nameWrap: 300,
