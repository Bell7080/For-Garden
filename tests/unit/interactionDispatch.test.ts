...
describe("interaction dispatch rules", () => {
  it("연구 레벨과 1~3명 고유 소유 편성을 검증한다", () => { expect(isInteractionCityUnlocked(city, city.unlock.researchLevel - 1)).toBe(false); expect(isInteractionCityUnlocked(city, city.unlock.researchLevel)).toBe(true); const owned = new Set(["a", "b", "c"]); expect(validateInteractionFormation([], owned)).toBe("party_size"); expect(validateInteractionFormation(["a", "a"], owned)).toBe("duplicate"); expect(validateInteractionFormation(["x"], owned)).toBe("not_owned"); expect(validateInteractionFormation(["a", "b", "c"], owned)).toBeNull(); });
  it("나이트 기어와 wind 태그가 이름 없이 시간과 보상에 반영된다", () => { const members = [{ id: "any-id", element: "wind" as const, squad: "gear" as const, tags: ["night-gear"] }]; expect(interactionDurationMs(city, members)).toBeLessThan(city.durationMinutes * 60_000); expect(interactionRewardWeights(city.rewards, members)).toEqual([6.25, 2]); });
