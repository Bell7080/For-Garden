...
interface SkillBase {
   ...
   * 패시브). 여기 적는 것은 지워지는 순간에 무엇이 함께 오는가뿐이다 — 일반 공격과 궁극기가
   * 서로 다르기 때문에 태그가 아니라 스킬이 든다.
   */
  shimmerBurst?: {
    /** 터지는 범위 피해의 주문력 계수(%). */
    power: number;
...
export type BasicAttackStep = {
  ...
  /**
   * 이 걸음이 적중한 순간 시전자가 이 시간(초)만큼 [[stealth|은신]]한다.
   *
   * 패시브의 은신(전투당 한 번, 저체력)과 다른 축이다 — 이쪽은 **순환의 한 걸음**이라 조건
   * 없이 되풀이되고 그만큼 짧다. 앞에 서야 값이 도는 개체가 그 자리에서 계속 맞고만 있지
   * 않도록 어그로를 주기적으로 끊는 용도다.
   */
  selfStealthSeconds?: number;
  /**
...
export type Ultimate = Skill & {
  ...
  /**
   * 같은 자리를 **연달아 내리찍는** 궁극기인가. 시전 순간이 곧 첫 번째다.
   *
   * 채널링과 다른 축이다 — 그쪽은 정해진 시간 동안 전장에 계속 떨어지는 것이고, 이쪽은
   * 정해진 횟수만큼 같은 범위를 다시 찍는 한 번의 동작이다. 그래서 총 위력이 아니라
   * **한 번의 위력**을 `power`에 적고, 게이지는 시전한 그 한 번의 몫만 쓴다.
   */
  repeatStrike?: {
    /** 시전 순간을 포함한 총 타격 횟수. */
    count: number;
  ...
  /**
   * 쓰는 순간 **방어력이 가장 낮은 적**에게 순간이동해 그 적에게 쓰는가.
   *
   * 표적을 고르는 일까지 궁극기가 맡는 이유는, 약점을 뚫는 기술이 정작 가장 단단한 적을
   * 때리고 있으면 이름이 하는 말과 결과가 갈리기 때문이다.
   */
...
export type PassiveKind =
  ...
  | "gourmetHunt"
  /** 티아 전용: 타격한 적에게 「반짝!」 표식을 남기고 추가 마법 피해를 준다. */
  | "shimmerMark"
...
export type FerocityEffectId =
  ...
  | "packHunt"
  /** 티아 전용: 폭주 중 매초 최대 체력을 되찾고 공격 속도를 올린다. */
  | "tidalVigor"
...
export type FerocityTrait = {
  ...
  | {
      ...
      bonusPercent: number;
      /**
       * 폭주 중 이 개체가 회복시킨 양의 이 비율(%)만큼 받은 쪽에 보호막을 덧씌운다.
       *
       * 회복은 이미 깎인 몫을 되돌리는 값이라 가득 찬 아군에게는 아무 일도 하지 않는다.
       * 그 위에 한 겹을 더 얹어야 폭주가 "더 많이 고쳤다"가 아니라 "더 오래 버티게 했다"로
       * 읽힌다.
       */
  ...
  | {
      ...
      /**
       * 폭주에 들어가는 순간 기본 공격의 상태 주기를 곧바로 채워 **다음 한 방**에 터지게 한다.
       *
       * 주기를 그대로 두면 폭주가 열린 직후 두세 대는 확정 치명타도 날려버림도 없이 지나가,
       * 보상 구간이 가장 뜨거워야 할 첫 순간이 비어 버린다. 세는 값을 건드리는 것이라
       * 특성 쪽 필드로 선언해 두고, 다른 개체가 같은 폭주를 갖게 되어도 데이터로 고른다.
       */
      loadsStatusCycleOnEntry: boolean;
  ...
  | {
      /**
       * 물살을 타고 계속 뛰어드는 몸이 버틸 숨과 손이다. 폭주 동안에만 돈다.
       *
       * 토리카의 폭주와 같은 1초 시계를 쓰되(`torikaBulwark`) 손이 함께 빨라진다 — 그쪽은
       * 앞에 서서 버티는 값이고, 이쪽은 계속 때리면서 버티는 값이다.
       */
      effectId: "tidalVigor";
...
export interface Passive {
   ...
   * 흡혈에 더하는 퍼센트포인트.
   *
   * 곱이 아니라 덧셈인 이유는 흡혈이 **전 개체 공통으로 0**이기 때문이다(`COMMON_SECONDARY_STATS`).
   ...
   *
   * 개체가 아니라 **거리**가 조건이다 — 원거리에서 쏘는 상대에게는 한두 대 맞으며 달려간
   * 끝에 파고들고, 상대도 중거리면 서로 닿는 그 순간 함께 뛰어드는 그림이 된다. 표적의
   * 사거리 등급으로 가르면 같은 거리에서도 어떤 판에서는 달려들고 어떤 판에서는 그냥 걷는다.
   */
  openingCharge?: {
    /** 이 사거리 등급이 정한 거리 안에 들어오면 발동한다. */
    withinReachTier: ReachTier;
    /** 파고든 자리에서 표적에게 거는 기절 시간(초). */
    stunSeconds: number;
  };
  ...
  /**
   * 체력이 최대의 절반 이하로 내려가는 순간 **전투당 한 번** 드는 은신(초)이다.
   *
   * 스테라의 `lowHpVanish`와 **같은 경계·같은 발동권**을 쓰되, 그쪽은 패시브 전체가 이
   * 규칙인 반면 여기서는 다른 패시브에 얹히는 한 절이다. 그래서 종류가 아니라 이 필드가
   * 있는지만 보고 돈다 — 개체 이름으로 가르면 개체가 늘 때마다 분기가 늘어난다.
   */
  lowHpVanishSeconds?: number;
  /**
   * 그 은신이 치르는 값. 참이면 그때까지 쌓은 [[focus|집중]]을 전부 잃는다.
   *
   * 공짜로 빠져나오면 뒤로 물러서는 것이 기본 행동이 되어 앞에 선 아군이 할 일이 없다.
   * 쌓아 둔 것을 내려놓고 사는 자리라, 살아남은 뒤에는 다시 쌓아 올려야 한다.
   */
  lowHpVanishSpendsFocus?: boolean;
  /**
  ...
  /**
   * 무면허 안전제일 전용: 자신이 울린 [[뇌진탕]]이 실제로 깎은 피해의 이 비율(%)만큼 보호막을 얻는다.
   *
   * 맞은 쪽 최대 체력에서 나오는 값이라 때린 상대가 클수록 두껍게 둘러진다 — 상한이 큰 한
   * 방만 누르는 패시브라, 잔타를 버티는 몫은 이 막이 맡는다.
   */
  concussionShieldPercent?: number;
  /**
   * 무면허 안전제일 전용: 한 번의 뇌진탕으로 두를 수 있는 보호막의 상한(자기 최대 체력 %)이다.
   *
   * 뇌진탕은 **맞은 쪽 최대 체력** 비례라, 체력이 무한한 불사 관측 보스를 때리면 그 비율이
   * 곧 무한한 막이 된다 — 상한이 없으면 아무도 쓰러지지 않아 점수전 자체가 끝나지 않는다.
...
export interface SummonDef {
  ...
  /**
   * 전장에 세울 때만 쓰는 표시 배율이다. 전투 수치에는 영향을 주지 않는다.
   *
   * 편성원과 같은 키로 세우면 네발짐승은 통째로 뚱뚱해 보인다 — 원화가 세로보다 가로로 넓어
   * 키를 맞추는 순간 폭이 사람만큼 남지 않기 때문이다. 그래서 그 몫을 화면이 개체 이름으로
